...
uses
  Classes,
  CWException, CWStackConsts, CWGlobal, SysUtils;
...
type
  ...
  public
    ...
    constructor CreateError(const AErr: Integer; const AMsg: AnsiString); virtual;
    //
  ...
  TIdServeFile = function(ASocket: TIdStackSocketHandle; const AFileName: AnsiString): Int64;

  TIdPacketInfo = class
  protected
    FSourceIP: AnsiString;
    FSourcePort : TIdPort;
    FSourceIF: LongWord;
  ...
  public
    ...
    property SourceIP : AnsiString read FSourceIP write FSourceIP;
    ...
    property DestIP : AnsiString read FDestIP write FDestIP;
  ...
  protected
    ...
    function HostByName(const AHostName: AnsiString;
      const AIPVersion: TIdIPVersion = ID_DEFAULT_IP_VERSION): AnsiString; virtual; abstract;
    function MakeCanonicalIPv6Address(const AAddr: AnsiString): AnsiString;
    function ReadHostName: AnsiString; virtual; abstract;
    function GetLocalAddress: AnsiString;
    function GetLocalAddresses: TStrings;
  public
    function Accept(ASocket: TIdStackSocketHandle; var VIP: AnsiString; var VPort: TIdPort): TIdStackSocketHandle; overload;
    ...
    function WSTranslateSocketErrorMsg(const AErr: integer): AnsiString; virtual;
    ...
    function ResolveHost(const AHost: AnsiString;
    ...
    function ReceiveFrom(ASocket: TIdStackSocketHandle; var VBuffer: TIdBytes;
      var VIP: AnsiString; var VPort: TIdPort; var VIPVersion: TIdIPVersion): Integer; virtual; abstract;
    ...
    function SendTo(ASocket: TIdStackSocketHandle; const ABuffer: TIdBytes;
      const AOffset: Integer; const ASize: Integer; const AIP: AnsiString;
    ...
    function IsValidIPv4MulticastGroup(const Value: AnsiString): Boolean;
    function IsValidIPv6MulticastGroup(const Value: AnsiString): Boolean;
    ...
    procedure DropMulticastMembership(AHandle: TIdStackSocketHandle;
      const AGroupIP, ALocalIP : AnsiString; const AIPVersion: TIdIPVersion = ID_DEFAULT_IP_VERSION); virtual; abstract;
    ...
    // Properties
    //
    property HostName: AnsiString read ReadHostName;
...
constructor EIdSocketError.CreateError(const AErr: Integer; const AMsg: AnsiString);
begin
...
function TIdStack.Accept(ASocket: TIdStackSocketHandle; var VIP: AnsiString;
...
procedure TIdStack.GetPeerName(ASocket: TIdStackSocketHandle; var VIP: AnsiString;
  var VPort: TIdPort);
var
  LIPVersion: TIdIPVersion;
begin
...
procedure TIdStack.GetSocketName(ASocket: TIdStackSocketHandle; var VIP: AnsiString;
  var VPort: TIdPort);
var
  LIPVersion: TIdIPVersion;
begin
...
function TIdStack.GetLocalAddress: AnsiString;
var
...
function TIdStack.IsIP(AIP: AnsiString): Boolean;
var
  i: Integer;
begin
//
...
function TIdStack.MakeCanonicalIPv6Address(const AAddr: AnsiString): AnsiString;
begin
  Result := CWGlobal.MakeCanonicalIPv6Address(AAddr);
end;

function TIdStack.ResolveHost(const AHost: AnsiString;
  const AIPVersion: TIdIPVersion = ID_DEFAULT_IP_VERSION): AnsiString;
begin
  Result := '';
  case AIPVersion of
...
function TIdStack.WSTranslateSocketErrorMsg(const AErr: integer): AnsiString;
begin
...
function TIdStack.IsValidIPv4MulticastGroup(const Value: AnsiString): Boolean;
var
  LIP: AnsiString;
  LVal: Integer;
begin
...
function TIdStack.IsValidIPv6MulticastGroup(const Value: AnsiString): Boolean;
var
  LTmp : AnsiString;
begin
...
begin
  ...
  while LSize > 1 do
  begin
    LCRC := LCRC + CWGlobal.BytesToWord(AData, i);
    Dec(LSize, 2);
    Inc(i, 2);
  end;
