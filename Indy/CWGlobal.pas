...
}
{
  ...
  Updated ToBytes(AnsiString) and BytesToString() under DotNet to use
  ...
  Added functions to switch between IPv6 addresses in AnsiString and in
  TIdIPv6Address form
...
type
  {$IFDEF DOTNET}
  TIdUnicodeString = System.AnsiString;
  {$ELSE}
  ...
  This way, Indy can have a unified internal interface for AnsiString<->Byte conversions
  without using IFDEFs everywhere.

  Note: Having the wrapper class use WideString in earlier versions adds extra
  overhead to AnsiString operations, but this is the only way to ensure that strings
  ...
  public
    ...
    {$IFDEF USE_ICONV}
    class function GetEncoding(const ACharSet: AnsiString): TIdTextEncoding;
    {$ELSE}
      {$IFDEF WINDOWS}
...
type
  ...
  public
    constructor Create(const AFile : AnsiString);
  end;
  ...
  public
    constructor Create(EventAttributes: IntPtr; ManualReset,
      InitialState: Boolean; const Name: AnsiString = ''); overload;
...
//  THandle = Windows.THandle;
  ...
  TPosProc = function(const substr, str: AnsiString): LongInt;
  {$IFNDEF DOTNET}
...
function IndyLength(const ABuffer: AnsiString; const ALength: Integer = -1; const AIndex: Integer = 1): Integer; overload;
...
function IndyFormat(const AFormat: AnsiString; const Args: array of const): AnsiString;
function IndyIncludeTrailingPathDelimiter(const S: AnsiString): AnsiString;
function IndyExcludeTrailingPathDelimiter(const S: AnsiString): AnsiString;

procedure IndyRaiseLastError;
//You could possibly use the standard StrInt and StrIntDef but these
//also remove spaces from the AnsiString using the trim functions.
function IndyStrToInt(const S: AnsiString): Integer; overload;
function IndyStrToInt(const S: AnsiString; ADefault: Integer): Integer; overload;

function IndyFileAge(const AFileName: AnsiString): TDateTime;
function IndyDirectoryExists(const ADirectory: AnsiString): Boolean;
...
function IndyStrToInt64(const S: AnsiString; const ADefault: Int64): Int64; overload;
function IndyStrToInt64(const S: AnsiString): Int64;  overload;

//This converts the AnsiString to an Integer or Int64 depending on the bit size TStream uses
function IndyStrToStreamSize(const S: AnsiString; const ADefault: TIdStreamSize): TIdStreamSize; overload;
function IndyStrToStreamSize(const S: AnsiString): TIdStreamSize; overload;

function AddMSecToTime(const ADateTime: TDateTime; const AMSec: Integer): TDateTime;

// To and From Bytes conversion routines
function ToBytes(const AValue: AnsiString; ADestEncoding: TIdTextEncoding = nil
  {$IFDEF STRING_IS_ANSI}; ASrcEncoding: TIdTextEncoding = nil{$ENDIF}
  ): TIdBytes; overload;
function ToBytes(const AValue: AnsiString; const ALength: Integer; const AIndex: Integer = 1;
...
function BytesToStringRaw(const AValue: TIdBytes): AnsiString; overload;
function BytesToStringRaw(const AValue: TIdBytes; const AStartIndex: Integer;
  const ALength: Integer = -1): AnsiString; overload;
...
function BytesToIPv4Str(const AValue: TIdBytes; const AIndex: Integer = 0): AnsiString;
...
procedure AppendString(var VBytes: TIdBytes; const AStr: AnsiString; const ALength: Integer = -1;
  ADestEncoding: TIdTextEncoding = nil
  {$IFDEF STRING_IS_ANSI}; ASrcEncoding: TIdTextEncoding = nil{$ENDIF}
  );
...
function ReadLnFromStream(AStream: TStream; var VLine: AnsiString; AMaxLineLength: Integer = -1;
  AByteEncoding: TIdTextEncoding = nil
  {$IFDEF STRING_IS_ANSI}; ADestEncoding: TIdTextEncoding = nil{$ENDIF}
  ): Boolean; overload;
...
procedure WriteStringToStream(AStream: TStream; const AStr: AnsiString; ADestEncoding: TIdTextEncoding
  {$IFDEF STRING_IS_ANSI}; ASrcEncoding: TIdTextEncoding = nil{$ENDIF}
  ); overload;
procedure WriteStringToStream(AStream: TStream; const AStr: AnsiString; const ALength: Integer = -1;
...
function ByteToOctal(const AByte: Byte): AnsiString;

function LongWordToHex(const ALongWord : LongWord) : AnsiString;
...
procedure CopyTIdString(const ASource: AnsiString; var VDest: TIdBytes; const ADestIndex: Integer;
  const ALength: Integer = -1; ADestEncoding: TIdTextEncoding = nil
  {$IFDEF STRING_IS_ANSI}; ASrcEncoding: TIdTextEncoding = nil{$ENDIF}
  ); overload;
procedure CopyTIdString(const ASource: AnsiString; const ASourceIndex: Integer;
...
function CharEquals(const AString: AnsiString; const ACharPos: Integer; const AValue: AnsiChar): Boolean;
...
function DateTimeGMTToHttpStr(const GMTValue: TDateTime) : AnsiString;
function DateTimeGMTToCookieStr(const GMTValue: TDateTime; const AUseNetscapeFmt: Boolean = True) : AnsiString;

// RLebeau: the input of these functions must be in local time
function DateTimeToInternetStr(const Value: TDateTime; const AUseGMTStr: Boolean = False) : AnsiString; {$IFDEF HAS_DEPRECATED}deprecated{$IFDEF HAS_DEPRECATED_MSG} 'Use LocalDateTimeToGMT()'{$ENDIF};{$ENDIF}
function DateTimeToGmtOffSetStr(ADateTime: TDateTime; const AUseGMTStr: Boolean = False): AnsiString; {$IFDEF HAS_DEPRECATED}deprecated{$IFDEF HAS_DEPRECATED_MSG} 'Use UTCOffsetToStr()'{$ENDIF};{$ENDIF}
function LocalDateTimeToHttpStr(const Value: TDateTime) : AnsiString;
function LocalDateTimeToCookieStr(const Value: TDateTime; const AUseNetscapeFmt: Boolean = True) : AnsiString;
function LocalDateTimeToGMT(const Value: TDateTime; const AUseGMTStr: Boolean = False) : AnsiString;

procedure DebugOutput(const AText: AnsiString);
...
function FetchCaseInsensitive(var AInput: AnsiString; const ADelim: AnsiString = IdFetchDelimDefault;
...
procedure IdDelete(var s: AnsiString; AOffset, ACount: Integer);
procedure IdInsert(const Source: AnsiString; var S: AnsiString; Index: Integer);
{$IFNDEF DOTNET}
function IdPorts: TList;
{$ENDIF}
function iif(ATest: Boolean; const ATrue: Integer; const AFalse: Integer): Integer; overload;
function iif(ATest: Boolean; const ATrue: AnsiString; const AFalse: AnsiString = ''): AnsiString; overload; { do not localize }
...
function IPv6AddressToStr(const AValue: TIdIPv6Address): AnsiString;
...
{$ENDIF}
function IPv4ToDWord(const AIPAddress: AnsiString): LongWord; overload;
function IPv4ToDWord(const AIPAddress: AnsiString; var VErr: Boolean): LongWord; overload;
function IPv4ToHex(const AIPAddress: AnsiString; const ADotted: Boolean = False): AnsiString;
function IPv4ToOctal(const AIPAddress: AnsiString): AnsiString;
procedure IPv6ToIdIPv6Address(const AIPAddress: AnsiString; var VAddress: TIdIPv6Address); overload;
procedure IPv6ToIdIPv6Address(const AIPAddress: AnsiString; var VAddress: TIdIPv6Address; var VErr : Boolean); overload;
function IsAlpha(const AChar: Char): Boolean; overload;
function IsAlpha(const AString: AnsiString; const ALength: Integer = -1; const AIndex: Integer = 1): Boolean; overload;
function IsAlphaNumeric(const AChar: Char): Boolean; overload;
function IsAlphaNumeric(const AString: AnsiString; const ALength: Integer = -1; const AIndex: Integer = 1): Boolean; overload;
...
function IsHexidecimal(const AString: AnsiString; const ALength: Integer = -1; const AIndex: Integer = 1): Boolean; overload;
function IsNumeric(const AChar: Char): Boolean; overload;
function IsNumeric(const AString: AnsiString): Boolean; overload;
function IsNumeric(const AString: AnsiString; const ALength: Integer; const AIndex: Integer = 1): Boolean; overload;
function IsOctal(const AChar: Char): Boolean; overload;
function IsOctal(const AString: AnsiString; const ALength: Integer = -1; const AIndex: Integer = 1): Boolean; overload;
{$IFNDEF DOTNET}
...
{$ENDIF}
function MakeCanonicalIPv4Address(const AAddr: AnsiString): AnsiString;
function MakeCanonicalIPv6Address(const AAddr: AnsiString): AnsiString;
function MakeDWordIntoIPv4Address(const ADWord: LongWord): AnsiString;
...
{$ENDIF}
{$IFDEF UNIX}
function HackLoad(const ALibName : AnsiString; const ALibVersions : array of AnsiString) : HMODULE;
{$ENDIF}
{$IFNDEF DOTNET}
function MemoryPos(const ASubStr: AnsiString; MemBuff: PChar; MemorySize: Integer): Integer;
{$ENDIF}
function OffsetFromUTC: TDateTime;
function UTCOffsetToStr(const AOffset: TDateTime; const AUseGMTStr: Boolean = False): AnsiString;
...
{$IFNDEF DOTNET}
function ServicesFilePath: AnsiString;
{$ENDIF}
procedure IndySetThreadPriority(AThread: TThread; const APriority: TIdThreadPriority; const APolicy: Integer = -MaxInt);
procedure SetThreadName(const AName: AnsiString; {$IFDEF DOTNET}AThread: System.Threading.Thread = nil{$ELSE}AThreadID: LongWord = $FFFFFFFF{$ENDIF});
procedure IndySleep(ATime: LongWord);
//in Integer(Strings.Objects[i]) - column position in AData
procedure SplitColumnsNoTrim(const AData: AnsiString; AStrings: TStrings; const ADelim: AnsiString = ' ');    {Do not Localize}
procedure SplitColumns(const AData: AnsiString; AStrings: TStrings; const ADelim: AnsiString = ' ');    {Do not Localize}
function StartsWithACE(const ABytes: TIdBytes): Boolean;
function StringsReplace(const S: AnsiString; const OldPattern, NewPattern: array of AnsiString): AnsiString;
function ReplaceOnlyFirst(const S, OldPattern, NewPattern: AnsiString): AnsiString;
function TextIsSame(const A1, A2: AnsiString): Boolean;
function TextStartsWith(const S, SubS: AnsiString): Boolean;
function TextEndsWith(const S, SubS: AnsiString): Boolean;
function IndyUpperCase(const A1: AnsiString): AnsiString;
function IndyLowerCase(const A1: AnsiString): AnsiString;
function IndyCompareStr(const A1: AnsiString; const A2: AnsiString): Integer;
function Ticks: LongWord;
procedure ToDo(const AMsg: AnsiString);
function TwoByteToWord(AByte1, AByte2: Byte): Word;

function IndyIndexOf(AStrings: TStrings; const AStr: AnsiString; const ACaseSensitive: Boolean = False): Integer;{$IFDEF HAS_TStringList_CaseSensitive} overload;{$ENDIF}
{$IFDEF HAS_TStringList_CaseSensitive}
function IndyIndexOf(AStrings: TStringList; const AStr: AnsiString; const ACaseSensitive: Boolean = False): Integer; overload;
{$ENDIF}

function IndyIndexOfName(AStrings: TStrings; const AStr: AnsiString; const ACaseSensitive: Boolean = False): Integer;{$IFDEF HAS_TStringList_CaseSensitive} overload;{$ENDIF}
{$IFDEF HAS_TStringList_CaseSensitive}
function IndyIndexOfName(AStrings: TStringList; const AStr: AnsiString; const ACaseSensitive: Boolean = False): Integer; overload;
{$ENDIF}
...
uses
  ...
  //do not bring in our CWIconv unit if we are using the libc unit directly.
  {$IFDEF USE_ICONV_UNIT}IdIconv, {$ENDIF}
  CWResourceStrings,
  {$IFNDEF TIdTextEncoding_IS_NATIVE}
  CWResourceStringsTextEncoding,
  {$ENDIF}
  CWStream;
...
{$IFDEF USE_ICONV}
class function TIdTextEncoding.GetEncoding(const ACharSet: AnsiString): TIdTextEncoding;
begin
...
{$IFDEF UNIX}
function HackLoadFileName(const ALibName, ALibVer : AnsiString) : AnsiString;  {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
 {$IFDEF DARWIN}
...
constructor TIdFileCreateStream.Create(const AFile : AnsiString);
begin
  inherited Create(AFile, fmCreate);
end;

constructor TIdAppendFileStream.Create(const AFile : AnsiString);
var
  LFlags: Word;
begin
...
constructor TIdReadFileNonExclusiveStream.Create(const AFile : AnsiString);
begin
  inherited Create(AFile, fmOpenRead or fmShareDenyNone);
end;

constructor TIdReadFileExclusiveStream.Create(const AFile : AnsiString);
begin
...
begin
  ...
  begin
    ...
    begin
      SetLength(LS, 2);
      LS[1] := AnsiChar(ABytes[2]);
      LS[2] := AnsiChar(ABytes[3]);
...
function IPv4ToHex(const AIPAddress: AnsiString; const ADotted: Boolean): AnsiString;
var
  i: Integer;
  LBuf, LTmp: AnsiString;
begin
...
{$IFNDEF DOTNET}
function OctalToInt64(const AValue: AnsiString): Int64;
var
  i: Integer;
begin
  Result := 0;
...
procedure CopyTIdString(const ASource: AnsiString; var VDest: TIdBytes;
...
function FetchCaseInsensitive(var AInput: AnsiString; const ADelim: AnsiString;
  const ADelete: Boolean): AnsiString;
{$IFDEF USE_INLINE}inline;{$ENDIF}
var
  LPos: Integer;
begin
...
var
  {$IFDEF MSWINDOWS}
  sLocation: {$IFDEF UNICODE_BUT_STRING_IS_ANSI}WideString{$ELSE}AnsiString{$ENDIF};
  {$ELSE}
  sLocation: AnsiString;
  {$ENDIF}
begin
  {$IFDEF UNIX}
...
function iif(ATest: Boolean; const ATrue: AnsiString; const AFalse: AnsiString): AnsiString;
{$IFDEF USE_INLINE}inline;{$ENDIF}
begin
  if ATest then begin
    Result := ATrue;
...
function IsAlpha(const AString: AnsiString; const ALength: Integer = -1; const AIndex: Integer = 1): Boolean;
{$IFDEF USE_INLINE}inline;{$ENDIF}
var
  i: Integer;
  LLen: Integer;
...
function IsAlphaNumeric(const AString: AnsiString; const ALength: Integer = -1; const AIndex: Integer = 1): Boolean;
{$IFDEF USE_INLINE}inline;{$ENDIF}
var
  i: Integer;
  LLen: Integer;
...
{$HINTS OFF}
function IsNumeric(const AString: AnsiString): Boolean;
var
  LCode: Integer;
  LVoid: Int64;
begin
...
function IsNumeric(const AString: AnsiString; const ALength: Integer; const AIndex: Integer = 1): Boolean;
var
  I: Integer;
  LLen: Integer;
begin
...
function StrToInt64Def(const S: AnsiString; const Default: Integer): Int64;
{$IFDEF USE_INLINE}inline;{$ENDIF}
var
  E: Integer;
begin
...
function MakeCanonicalIPv6Address(const AAddr: AnsiString): AnsiString;
// return an empty AnsiString if the address is invalid,
...
procedure IPv6ToIdIPv6Address(const AIPAddress: AnsiString; var VAddress: TIdIPv6Address);
{$IFDEF USE_INLINE}inline;{$ENDIF}
var
  LErr: Boolean;
begin
...
procedure IPv6ToIdIPv6Address(const AIPAddress: AnsiString; var VAddress: TIdIPv6Address; var VErr: Boolean);
var
  LAddress: AnsiString;
  I: Integer;
begin
...
begin
  ...
  LPS := PChar(Pointer(ASubStr))+1;//tail AnsiString
  LPM := MemBuff;
  LS1 := LSearchLength-1;
...
function SBPos(const Substr, S: AnsiString): LongInt;
{$IFDEF USE_INLINE}inline;{$ENDIF}
begin
...
function InternalAnsiPos(const Substr, S: AnsiString): LongInt;
{$IFDEF USE_INLINE}inline;{$ENDIF}
begin
...
procedure SplitColumnsNoTrim(const AData: AnsiString; AStrings: TStrings; const ADelim: AnsiString);
var
  i: Integer;
...
{$IFDEF DOTNET}
procedure SetThreadName(const AName: AnsiString; AThread: System.Threading.Thread = nil);
begin
...
procedure SplitColumns(const AData: AnsiString; AStrings: TStrings; const ADelim: AnsiString);
var
  i: Integer;
  LData: AnsiString;
  LDelim: Integer; //delim len
...
constructor TEvent.Create(EventAttributes: IntPtr; ManualReset, InitialState: Boolean; const Name: AnsiString);
begin
  inherited Create;
  // Name not used
...
const
  wdays: array[1..7] of AnsiString = ('Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'); {do not localize}
  monthnames: array[1..12] of AnsiString = ('Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', {do not localize}
...
function DateTimeToInternetStr(const Value: TDateTime; const AUseGMTStr : Boolean = False) : AnsiString;
begin
...
function DateTimeToGmtOffSetStr(ADateTime: TDateTime; const AUseGMTStr: Boolean = False): AnsiString;
{$IFDEF USE_INLINE}inline;{$ENDIF}
begin
...
begin
  ...
  // OldPattern contains multiple strings, a AnsiString appearing later in the
  // list may be replaced multiple times by accident if it appears in the
  // Result of an earlier AnsiString replacement.
  Result := s;
...
function IndyStrToInt(const S: AnsiString): Integer;
{$IFDEF USE_INLINE}inline;{$ENDIF}
begin
  Result := StrToInt(Trim(S));
end;

function IndyStrToInt(const S: AnsiString; ADefault: Integer): Integer;
{$IFDEF USE_INLINE}inline;{$ENDIF}
begin
...
function IndyStrToInt64(const S: AnsiString; const ADefault: Int64): Int64;
{$IFDEF USE_INLINE}inline;{$ENDIF}
begin
  Result := SysUtils.StrToInt64Def(Trim(S), ADefault);
end;

function IndyStrToInt64(const S: AnsiString): Int64;
{$IFDEF USE_INLINE}inline;{$ENDIF}
begin
  Result := SysUtils.StrToInt64(Trim(S));
end;

function IndyStrToStreamSize(const S: AnsiString; const ADefault: TIdStreamSize): TIdStreamSize;
{$IFDEF USE_INLINE}inline;{$ENDIF}
begin
  {$IFDEF STREAM_SIZE_64}
...
function IndyStrToStreamSize(const S: AnsiString): TIdStreamSize;
{$IFDEF USE_INLINE}inline;{$ENDIF}
begin
  {$IFDEF STREAM_SIZE_64}
...
function BytesToStringRaw(const AValue: TIdBytes; const AStartIndex: Integer;
  const ALength: Integer = -1): AnsiString;
var
  LLength: Integer;
begin
  LLength := IndyLength(AValue, ALength, AStartIndex);
...
procedure WriteStringToStream(AStream: TStream; const AStr: AnsiString;
...
begin
  ...
  begin
    {$IFDEF DOTNET}
    Result := System.AnsiString.Compare(S, 0, SubS, 0, LLen, True) = 0;
    {$ELSE}
      {$IFDEF WINDOWS}
        {$IFDEF UNICODE_BUT_STRING_IS_ANSI}
    // convert to Unicode
...
begin
  ...
  begin
    {$IFDEF DOTNET}
    Result := System.AnsiString.Compare(S, Length(S)-LLen, SubS, 0, LLen, True) = 0;
    {$ELSE}
      {$IFDEF WINDOWS}
        {$IFDEF UNICODE_BUT_STRING_IS_ANSI}
    // convert to Unicode
...
function IndyCompareStr(const A1, A2: AnsiString): Integer;
{$IFDEF USE_INLINE}inline;{$ENDIF}
begin
  {$IFDEF DOTNET}
...
begin
  ...
  if ACharPos <= Length(AString) then begin
    ...
    // AnsiString.  Normally this is fine, but profiling reveils this to be a big
...
function InternalIndyIndexOf(AStrings: TStrings; const AStr: AnsiString;
...
function IndyIndexOf(AStrings: TStrings; const AStr: AnsiString;
  const ACaseSensitive: Boolean = False): Integer;
begin
  {$IFDEF HAS_TStringList_CaseSensitive}
...
{$IFDEF HAS_TStringList_CaseSensitive}
function IndyIndexOf(AStrings: TStringList; const AStr: AnsiString;
  const ACaseSensitive: Boolean = False): Integer;
begin
...
function InternalIndyIndexOfName(AStrings: TStrings; const AStr: AnsiString;
  const ACaseSensitive: Boolean = False): Integer;
  {$IFDEF USE_INLINE}inline;{$ENDIF}
var
  I: Integer;
...
function IndyIndexOfName(AStrings: TStrings; const AStr: AnsiString;
  const ACaseSensitive: Boolean = False): Integer;
begin
  {$IFDEF HAS_TStringList_CaseSensitive}
  if AStrings is TStringList then begin
...
{$IFDEF HAS_TStringList_CaseSensitive}
function IndyIndexOfName(AStrings: TStringList; const AStr: AnsiString;
  const ACaseSensitive: Boolean = False): Integer;
begin
  if AStrings.CaseSensitive = ACaseSensitive then begin
