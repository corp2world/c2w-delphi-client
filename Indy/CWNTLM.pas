...
type
  ...
  public
    ...
    property dom_len1: Word read fdom_len1 write fdom_len1;                     // domain AnsiString length
    property dom_len2: Word read fdom_len2 write fdom_len2;                     // domain AnsiString length
    property dom_off: LongWord read fdom_off write fdom_off;                  // domain AnsiString offset

    property host_len1: Word read fhost_len1 write fhost_len1;                    // host AnsiString length
    property host_len2: Word read fhost_len2 write fhost_len2;                    // host AnsiString length
    property host_off: LongWord read fhost_off write fhost_off;                 // host AnsiString offset (always 0x20)
  end;
  ...
  public
    ...
    property dom_len1: Word read fdom_len1 write fdom_len1;           // domain AnsiString length
    property dom_len2: Word read fdom_len2 write fdom_len2;           // domain AnsiString length
    property dom_off: LongWord read fdom_off write fdom_off;          // domain AnsiString offset (always 0x40)

    property user_len1: Word read fuser_len1 write fuser_len1;                    // username AnsiString length
    property user_len2: Word read fuser_len2 write fuser_len2;                    // username AnsiString length
    property user_off: LongWord read fuser_off write fuser_off;                 // username AnsiString offset

    property host_len1: Word read fhost_len1 write fhost_len1;                    // host AnsiString length
    property host_len2: Word read fhost_len2 write fhost_len2;                    // host AnsiString length
    property host_off: LongWord read fhost_off write fhost_off;                 // host AnsiString offset
...
{$ELSE}
  type_1_message_header = packed record
    ...
    dom_len1: Word;                     // domain AnsiString length
    dom_len2: Word;                     // domain AnsiString length
    dom_off: LongWord;                  // domain AnsiString offset

    host_len1: Word;                    // host AnsiString length
    host_len2: Word;                    // host AnsiString length
    host_off: LongWord;                 // host AnsiString offset (always 0x20)
  end;
  ...
  type_3_message_header = packed record
    ...
    dom_len1: Word;                     // domain AnsiString length
    dom_len2: Word;                     // domain AnsiString length
    dom_off: LongWord;                  // domain AnsiString offset (always 0x40)

    user_len1: Word;                    // username AnsiString length
    user_len2: Word;                    // username AnsiString length
    user_off: LongWord;                 // username AnsiString offset

    host_len1: Word;                    // host AnsiString length
    host_len2: Word;                    // host AnsiString length
    host_off: LongWord;                 // host AnsiString offset
...
function BuildType1Message(const ADomain, AHost: AnsiString): AnsiString;
function BuildType3Message(const ADomain, AHost, AUsername: TIdUnicodeString; const APassword: AnsiString; const ANonce: TIdBytes): AnsiString;
{$ENDIF}

function NTLMFunctionsLoaded : Boolean;

procedure GetDomain(const AUserName : AnsiString; var VUserName, VDomain : AnsiString);
...
uses
  ...
  CWHashMessageDigest,
  CWCoderMIME
  {$IFNDEF DOTNET}, CWSSLOpenSSLHeaders{$ENDIF}
  ;
...
function SetupLanManagerPassword(const APassword: AnsiString; const ANonce: TIdBytes): TIdBytes;
var
...
function BuildUnicode(const S: AnsiString): TIdBytes;
{$IFDEF STRING_IS_UNICODE}
  {$IFDEF USE_INLINE}inline;{$ENDIF}
{$ELSE}
var
...
begin
  ...
  // as well?  This logic will not produce a valid Unicode AnsiString if
...
function CreateNTPassword(const APassword: AnsiString; const ANonce: TIdBytes): TIdBytes;
var
...
function BuildType3Message(const ADomain, AHost, AUsername: TIdUnicodeString;
  const APassword: AnsiString; const ANonce: TIdBytes): AnsiString;
var
