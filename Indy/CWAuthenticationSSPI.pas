...
type
  ...
  public
    ...
    constructor CreateError(const AErrorNo: Integer; const AFailedFuncName: AnsiString);
    //
    class function GetErrorMessageByNo(AErrorNo: LongWord): AnsiString;
  end;
  ...
  public
    class procedure RaiseIfError(aStatus: SECURITY_STATUS; const aFunctionName: AnsiString);
  ...
  private
    ...
    procedure UpdateHasContextAndCheckForError(
      const aFuncResult: SECURITY_STATUS; const aFuncName: AnsiString;
  ...
  public
    function GenerateInitialChallenge(const aTargetName: AnsiString;
  ...
  public
    procedure SetCredentials(const aDomain, aUserName, aPassword: AnsiString);
...
class function ESSPIException.GetErrorMessageByNo(aErrorNo: LongWord): AnsiString;
begin
...
constructor ESSPIException.CreateError(const AErrorNo: Integer; const AFailedFuncName: AnsiString);
begin
...
function TSSPIClientConnectionContext.GenerateInitialChallenge
  (const aTargetName: AnsiString; var aToPeerToken: TIdBytes): Boolean;
begin
  Release;
...
procedure TIndySSPINTLMClient.SetCredentials(const aDomain, aUserName, aPassword: AnsiString);
begin
...
function TIdSSPINTLMAuthentication.Authentication: AnsiString;
var
  buf: TIdBytes;
begin
  Result := '';
...
procedure TIdSSPINTLMAuthentication.SetDomain(const Value: AnsiString);
begin
  Params.Values['Domain'] := Value; {do not localize}
end;

function TIdSSPINTLMAuthentication.GetDomain: AnsiString;
begin
  Result := Params.Values['Domain']; {do not localize}
end;

procedure TIdSSPINTLMAuthentication.SetUserName(const Value: AnsiString);
Var
  S: AnsiString;
begin
  S := Value;
