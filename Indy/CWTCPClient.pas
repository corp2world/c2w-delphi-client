...
uses
  Classes,
  CWGlobal, CWExceptionCore, CWIOHandler, CWTCPConnection;
...
type
  ...
  protected
    ...
    procedure SetBoundIP(const AValue: AnsiString);
  ...
  public
    ...
    // params
    procedure Connect(const AHost: AnsiString); overload;
    procedure Connect(const AHost: AnsiString; const APort: TIdPort); overload;
    function ConnectAndGetAll: AnsiString; virtual;
    //
...
uses
  CWComponent, CWResourceStringsCore, CWIOHandlerSocket;
...
function TIdTCPClientCustom.ConnectAndGetAll: AnsiString;
begin
  Connect; try
...
procedure TIdTCPClientCustom.SetBoundIP(const AValue: AnsiString);
begin
  FBoundIP := AValue;
  if Socket <> nil then begin
...
procedure TIdTCPClientCustom.SetHost(const AValue: AnsiString);
begin
  FHost := AValue;
  if IOHandler <> nil then begin
...
procedure TIdTCPClientCustom.Connect(const AHost: AnsiString);
begin
  Host := AHost;
  Connect;
end;

procedure TIdTCPClientCustom.Connect(const AHost: AnsiString; const APort: TIdPort);
begin
  Host := AHost;
  Port := APort;
  Connect;
