...
type
  ...
  protected
    ...
    //function FoldLine(AString : AnsiString): TStrings; {$IFDEF HAS_DEPRECATED}deprecated{$IFDEF HAS_DEPRECATED_MSG} 'Use FoldLineToList()'{$ENDIF};{$ENDIF}
    procedure FoldLineToList(AString : AnsiString; ALines: TStrings);
    ...
    function GetAllParams(const AName: AnsiString): AnsiString;
    ...
    procedure SetAllParams(const AName, AValue: AnsiString);
    {Gets a value from a AnsiString}
    function GetValueFromLine(var VLine : Integer) : AnsiString;
  ...
  public
    ...
    procedure Extract(const AName: AnsiString; ADest: TStrings);
    ...
    property AllParams[const Name: AnsiString]: AnsiString read GetAllParams write SetAllParams;
...
procedure TIdHeaderList.AddValue(const AName, AValue: AnsiString);
var
  I: Integer;
begin
...
procedure TIdHeaderList.Extract(const AName: AnsiString; ADest: TStrings);
var
  idx : Integer;
begin
...
procedure TIdHeaderList.FoldAndInsert(AString : AnsiString; Index: Integer);
var
  LStrs : TStrings;
  idx : Integer;
begin
...
{function TIdHeaderList.FoldLine(AString : AnsiString): TStrings;
begin
...
procedure TIdHeaderList.FoldLineToList(AString : AnsiString; ALines: TStrings);
var
  s : AnsiString;
begin
...
function TIdHeaderList.GetName(Index: Integer): AnsiString;
var
  I : Integer;
begin
  Result := Get(Index);
...
function TIdHeaderList.GetValue(const AName: AnsiString): AnsiString;
var
  idx: Integer;
begin
...
function TIdHeaderList.GetValueFromLine(var VLine: Integer): AnsiString;
var
...
function TIdHeaderList.GetParam(const AName, AParam: AnsiString): AnsiString;
var
  s: AnsiString;
...
function TIdHeaderList.GetAllParams(const AName: AnsiString): AnsiString;
var
  s: AnsiString;
begin
  s := Values[AName];
...
function TIdHeaderList.IndexOfName(const AName: AnsiString): Integer;
var
  i: LongInt;
begin
  Result := -1;
...
procedure TIdHeaderList.SetValue(const AName, AValue: AnsiString);
var
  I: Integer;
begin
  I := IndexOfName(AName);
...
procedure TIdHeaderList.SetParam(const AName, AParam, AValue: AnsiString);
var
  LQuoteType: TIdHeaderQuotingType;
begin
  LQuoteType := FQuoteType;
...
procedure TIdHeaderList.SetAllParams(const AName, AValue: AnsiString);
var
  LValue: AnsiString;
begin
  LValue := Values[AName];
