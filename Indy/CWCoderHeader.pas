...
// Procs
  function EncodeAddressItem(EmailAddr: TIdEmailAddressItem; const HeaderEncoding: Char;
    const MimeCharSet: AnsiString; AUseAddressForNameIfNameMissing: Boolean = False): AnsiString;
  function EncodeHeader(const Header: AnsiString; Specials: AnsiString; const HeaderEncoding: Char;
   const MimeCharSet: AnsiString): AnsiString;
  function EncodeAddress(EmailAddr: TIdEMailAddressList; const HeaderEncoding: Char;
    const MimeCharSet: AnsiString; AUseAddressForNameIfNameMissing: Boolean = False): AnsiString;
  function DecodeHeader(const Header: AnsiString): AnsiString;
  procedure DecodeAddress(EMailAddr: TIdEmailAddressItem);
  procedure DecodeAddresses(AEMails: AnsiString; EMailAddr: TIdEmailAddressList);
...
const
  csAddressSpecials: AnsiString = '()[]<>:;.,@\"';  {Do not Localize}
...
function DecodeHeader(const Header: AnsiString): AnsiString;
var
  HeaderCharSet, HeaderEncoding, HeaderData, S: AnsiString;
  LDecoded: Boolean;
  LStartPos, LLength, LEncodingStartPos, LEncodingEndPos, LLastStartPos: Integer;
  LLastWordWasEncoded: Boolean;
  Buf: TIdBytes;

  function ExtractEncoding(const AHeader: AnsiString; const AStartPos: Integer;
    var VStartPos, VEndPos: Integer; var VCharSet, VEncoding, VData: AnsiString): Boolean;
  var
  ...
  function ExtractEncodedData(const AEncoding, AData: AnsiString; var VDecoded: TIdBytes): Boolean;
  var
    I, J: Integer;
    a3: TIdBytes;
...
begin
  ...
  begin
    ...
    begin
      ...
      if LDecoded then
      begin
        //replace old substring in header with decoded AnsiString,
...
var
  ...
  BeginEncode, EndEncode: AnsiString;
