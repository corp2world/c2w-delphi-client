...
//TODO: See second uses comment
 ...
 Added a resource AnsiString for the exception raised in
 ...
 Modified CheckResponse AnsiString version to allow ''
...
type
  ...
  protected
    ...
    procedure PrepareCmd(var aCmd: AnsiString); virtual;
  public
    ...
    function CheckResponse(const AResponse, AAllowedResponse: AnsiString): AnsiString; overload; virtual;
    //
    ...
    // in the future
    function GetResponse(const AAllowedResponse: AnsiString;
      AEncoding: TIdTextEncoding = nil): AnsiString; overload; virtual;
    //
    ...
    function SendCmd(AOut: AnsiString; const AResponse: SmallInt = -1;
      AEncoding: TIdTextEncoding = nil): SmallInt; overload;
    function SendCmd(AOut: AnsiString; const AResponse: array of SmallInt;
      AEncoding: TIdTextEncoding = nil): SmallInt; overload; virtual;
    function SendCmd(AOut: AnsiString; const AResponse: AnsiString;
      AEncoding: TIdTextEncoding = nil): AnsiString; overload;
    //
...
uses
  CWAntiFreezeBase, CWResourceStringsCore, CWStackConsts, CWReplyRFC,
  SysUtils;
...
function TIdTCPConnection.SendCmd(AOut: AnsiString; const AResponse: Array of SmallInt;
  AEncoding: TIdTextEncoding = nil): SmallInt;
begin
  CheckConnected;
  PrepareCmd(AOut);
...
function TIdTCPConnection.SendCmd(AOut: AnsiString; const AResponse: SmallInt = -1;
  AEncoding: TIdTextEncoding = nil): SmallInt;
begin
...
function TIdTCPConnection.GetResponse(const AAllowedResponse: AnsiString;
  AEncoding: TIdTextEncoding = nil): AnsiString;
begin
  GetInternalResponse(AEncoding);
  Result := CheckResponse(LastCmdResult.Code, AAllowedResponse);
end;

function TIdTCPConnection.SendCmd(AOut: AnsiString; const AResponse: AnsiString;
  AEncoding: TIdTextEncoding = nil): AnsiString;
begin
  CheckConnected;
  PrepareCmd(AOut);
...
function TIdTCPConnection.CheckResponse(const AResponse, AAllowedResponse: AnsiString): AnsiString;
begin
  if (AAllowedResponse <> '')
...
procedure TIdTCPConnection.PrepareCmd(var aCmd: AnsiString);
begin
