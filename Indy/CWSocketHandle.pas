...
uses
  Classes,
  CWException, CWGlobal, CWStackConsts, CWStack, CWBaseComponent;
...
type
  ...
  public
    ...
    procedure Bind;
    procedure Broadcast(const AData: AnsiString; const APort: TIdPort; const AIP: AnsiString = '';
      AByteEncoding: TIdTextEncoding = nil
      {$IFDEF STRING_IS_ANSI}; ASrcEncoding: TIdTextEncoding = nil{$ENDIF}
      ); overload;
    procedure Broadcast(const AData: TIdBytes; const APort: TIdPort; const AIP: AnsiString = ''); overload;
    ...
    function RecvFrom(var ABuffer : TIdBytes; var VIP: AnsiString;
      var VPort: TIdPort; var VIPVersion: TIdIPVersion): Integer;
    procedure Reset(const AResetLocal: boolean = True);
    function Send(const AData: AnsiString; AByteEncoding: TIdTextEncoding = nil
      {$IFDEF STRING_IS_ANSI}; ASrcEncoding: TIdTextEncoding = nil{$ENDIF}
      ): Integer; overload;
    function Send(const ABuffer: TIdBytes; const AOffset: Integer = 0; const ASize: Integer = -1): Integer; overload;
    procedure SendTo(const AIP: AnsiString; const APort: TIdPort; const AData: AnsiString;
      const AIPVersion: TIdIPVersion = ID_DEFAULT_IP_VERSION; AByteEncoding: TIdTextEncoding = nil
      {$IFDEF STRING_IS_ANSI}; ASrcEncoding: TIdTextEncoding = nil{$ENDIF}
      ); overload;
    procedure SendTo(const AIP: AnsiString; const APort: TIdPort; const ABuffer : TIdBytes; const AIPVersion: TIdIPVersion = ID_DEFAULT_IP_VERSION); overload;
    procedure SendTo(const AIP: AnsiString; const APort: TIdPort; const ABuffer : TIdBytes; const AOffset: Integer; const ASize: Integer; const AIPVersion: TIdIPVersion = ID_DEFAULT_IP_VERSION); overload;
    procedure SetPeer(const AIP: AnsiString; const APort: TIdPort; const AIPVersion : TIdIPVersion = ID_DEFAULT_IP_VERSION);
    procedure SetBinding(const AIP: AnsiString; const APort: TIdPort; const AIPVersion : TIdIPVersion = ID_DEFAULT_IP_VERSION);
    ...
    procedure AddMulticastMembership(const AGroupIP: AnsiString);
    procedure DropMulticastMembership(const AGroupIP: AnsiString);
...
uses
  ...
  System.SyncObjs,
  {$ENDIF}
  CWAntiFreezeBase, CWComponent, CWResourceStrings, SysUtils;
...
function TIdSocketHandle.Send(const AData: AnsiString; AByteEncoding: TIdTextEncoding = nil
  {$IFDEF STRING_IS_ANSI}; ASrcEncoding: TIdTextEncoding = nil{$ENDIF}
  ): Integer;
begin
...
procedure TIdSocketHandle.SendTo(const AIP: AnsiString; const APort: TIdPort;
  const AData: AnsiString; const AIPVersion: TIdIPVersion = ID_DEFAULT_IP_VERSION;
  AByteEncoding: TIdTextEncoding = nil
  {$IFDEF STRING_IS_ANSI}; ASrcEncoding: TIdTextEncoding = nil{$ENDIF}
  );
begin
...
function TIdSocketHandle.RecvFrom(var ABuffer : TIdBytes; var VIP: AnsiString;
 var VPort: TIdPort; var VIPVersion: TIdIPVersion): Integer;
begin
...
procedure TIdSocketHandle.Broadcast(const AData: AnsiString; const APort: TIdPort;
  const AIP: AnsiString = ''; AByteEncoding: TIdTextEncoding = nil
  {$IFDEF STRING_IS_ANSI}; ASrcEncoding: TIdTextEncoding = nil{$ENDIF}
  );
begin
...
procedure TIdSocketHandle.SetPeer(const AIP: AnsiString; const APort: TIdPort; const AIPVersion : TIdIPVersion = ID_DEFAULT_IP_VERSION);
begin
  FPeerIP := AIP;
  FPeerPort := APort;
...
procedure TIdSocketHandle.SetBinding(const AIP: AnsiString; const APort: TIdPort; const AIPVersion : TIdIPVersion = ID_DEFAULT_IP_VERSION);
begin
  FIP := AIP;
  FPort := APort;
  FIPVersion := AIPVersion;
...
procedure TIdSocketHandle.AddMulticastMembership(const AGroupIP: AnsiString);
begin
  GStack.AddMulticastMembership(Handle, AGroupIP, FIP, FIPVersion);
end;

procedure TIdSocketHandle.DropMulticastMembership(const AGroupIP: AnsiString);
begin
