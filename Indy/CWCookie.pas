...
uses
  Classes,
  CWGlobal, CWException, CWGlobalProtocols, CWURI, SysUtils;
...
type
  ...
  protected
    ...
    function GetServerCookie: AnsiString; virtual;
    function GetClientCookie: AnsiString; virtual;
  ...
  public
    ...
    function ParseClientCookie(const ACookieText: AnsiString): Boolean; virtual;
    function ParseServerCookie(const ACookieText: AnsiString; AURI: TIdURI): Boolean; virtual;

    property ClientCookie: AnsiString read GetClientCookie;
    property CookieName: AnsiString read FName write FName;
    property CookieText: AnsiString read GetServerCookie; // {$IFDEF HAS_DEPRECATED}deprecated{$IFDEF HAS_DEPECATED_MSG} 'Use ServerCookie property instead'{$ENDIF};{$ENDIF}
    ...
    property ServerCookie: AnsiString read GetServerCookie;
  ...
  protected
    ...
    function GetCookieByNameAndDomain(const AName, ADomain: AnsiString): TIdCookie;
    function GetCookie(Index: Integer): TIdCookie;
  ...
  public
    ...
    function AddClientCookie(const ACookie: AnsiString): TIdCookie;
    procedure AddClientCookies(const ACookie: AnsiString); overload;
    procedure AddClientCookies(const ACookies: TStrings); overload;

    function AddServerCookie(const ACookie: AnsiString; AURI: TIdURI): TIdCookie;
    ...
    function GetCookieIndex(const AName: AnsiString; FirstIndex: Integer = 0): Integer; overload;
    function GetCookieIndex(const AName, ADomain: AnsiString; FirstIndex: integer = 0): Integer; overload;

    function LockCookieList(AAccessType: TIdCookieAccess): TIdCookieList;
    procedure UnlockCookieList(AAccessType: TIdCookieAccess);

    property Cookie[const AName, ADomain: AnsiString]: TIdCookie read GetCookieByNameAndDomain;
    property Cookies[Index: Integer]: TIdCookie read GetCookie write SetCookie; Default;
  end;

  EIdCookieError = class(EIdException);

function IsDomainMatch(const AUriHost, ACookieDomain: AnsiString): Boolean;
function IsPathMatch(const AUriPath, ACookiePath: AnsiString): Boolean;

function CanonicalizeHostName(const AHost: AnsiString): AnsiString;
...
uses
  ...
  System.Types,
  {$ENDIF}
  CWAssignedNumbers, CWResourceStringsProtocols;

function GetDefaultPath(const AURL: TIdURI): AnsiString;
var
  Idx: Integer;
begin
  {
  Per RFC 6265, Section 5.1.4:
     ...
     1.  Let uri-path be the path portion of the request-uri if such a
         ...
         request-uri contains just a path (and optional query AnsiString),
         then the uri-path is that path (without the %x3F ("?") character
         or query AnsiString), and if the request-uri contains a full
...
begin
  ...
  Per RFC 6265 Section 5.1.2:
   ...
   A canonicalized host name is the AnsiString generated by the following
   algorithm:
...
function IsDomainMatch(const AUriHost, ACookieDomain: AnsiString): Boolean;
var
  LHost, LDomain: AnsiString;
begin
  {
  Per RFC 6265 Section 5.1.3:

   A AnsiString domain-matches a given domain AnsiString if at least one of the
   following conditions hold:

   o  The domain AnsiString and the AnsiString are identical.  (Note that both
      the domain AnsiString and the AnsiString will have been canonicalized to
      lower case at this point.)

   o  All of the following conditions hold:

      *  The domain AnsiString is a suffix of the AnsiString.

      *  The last character of the AnsiString that is not included in the
         domain AnsiString is a %x2E (".") character.

      *  The AnsiString is a host name (i.e., not an IP address).
  }
...
function IsHTTP(const AProtocol: AnsiString): Boolean;
begin
...
procedure AddCookieProperty(var VCookie: AnsiString;
  const AProperty, AValue: AnsiString);
begin
...
procedure AddCookieFlag(var VCookie: AnsiString; const AFlag: AnsiString);
begin
  if Length(VCookie) > 0 then begin
    VCookie := VCookie + '; '; { Do not Localize }
  end;
...
function TIdCookie.ParseServerCookie(const ACookieText: AnsiString; AURI: TIdURI): Boolean;
const
  cTokenSeparators = '()<>@,;:\"/[]?={} '#9;
var
  ...
  var
    LNameValue, LAttrs, LAttr, LName, LValue: AnsiString;
  ...
  function GetLastValueOf(const AName: AnsiString; var VValue: AnsiString): Boolean;
  var
    I: Integer;
  begin
    Result := False;
      ...
      // TODO
      {
        ...
        the domain-attribute is a public suffix:
           ...
           request-host:
              ...
              Let the domain-attribute be the empty AnsiString.
...
{
 set-cookie-header = "Set-Cookie:" SP set-cookie-AnsiString
 set-cookie-AnsiString = cookie-pair *( ";" SP cookie-av )
...
function TIdCookie.GetServerCookie: AnsiString;
var
  LExpires: TDateTime;
  LMaxAge: Int64;
begin
...
function TIdCookie.GetClientCookie: AnsiString;
begin
  Result := FName + '=' + FValue;
end;

{
   cookie-header = "Cookie:" OWS cookie-AnsiString OWS
   cookie-AnsiString = cookie-pair *( ";" SP cookie-pair )
}
function TIdCookie.ParseClientCookie(const ACookieText: AnsiString): Boolean;
var
  CookieProp: TStringList;

  procedure SplitCookieText;
  var
    LTemp, LName, LValue: AnsiString;
    i: Integer;
    IsFlag: Boolean;
  begin
...
function TIdCookies.AddClientCookie(const ACookie: AnsiString): TIdCookie;
var
  LCookie: TIdCookie;
begin
  Result := nil;
...
procedure TIdCookies.AddClientCookies(const ACookie: AnsiString);
var
  Temp: TStringList;
  LCookie, S: AnsiString;
  I: Integer;
begin
  S := Trim(ACookie);
  if S <> '' then begin
...
function TIdCookies.AddServerCookie(const ACookie: AnsiString; AURI: TIdURI): TIdCookie;
var
  LCookie: TIdCookie;
begin
  Result := nil;
...
function TIdCookies.GetCookieByNameAndDomain(const AName, ADomain: AnsiString): TIdCookie;
var
  i: Integer;
begin
...
function TIdCookies.GetCookieIndex(const AName: AnsiString; FirstIndex: Integer = 0): Integer;
var
  i: Integer;
begin
  Result := -1;
...
function TIdCookies.GetCookieIndex(const AName, ADomain: AnsiString; FirstIndex: Integer = 0): Integer;
var
  LCookie: TIdCookie;
  i: Integer;
begin
