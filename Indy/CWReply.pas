...
}
{
  ...
  Added formatted resource AnsiString for the exception raised in
  TIdReply.SetCode.
...
type
  ...
  protected
    ...
    function CheckIfCodeIsValid(const ACode: AnsiString): Boolean; virtual;
    function GetDisplayName: string; override;
    function GetFormattedReply: TStrings; virtual;
    function GetNumericCode: Integer;
    procedure SetCode(const AValue: AnsiString);
  ...
  public
   ...
   // constructor Create(ACollection: TCollection; AReplyTexts: TIdReplies); reintroduce; overload; virtual;
    ...
    class function IsEndMarker(const ALine: AnsiString): Boolean; virtual;
    procedure RaiseReplyError; virtual; abstract;
    function ReplyExists: Boolean; virtual;
    procedure SetReply(const ACode: Integer; const AText: AnsiString); overload; virtual;
    procedure SetReply(const ACode: AnsiString; const AText: AnsiString); overload; virtual;
  ...
  public
    ...
    function Add(const ACode: Integer; const AText: AnsiString): TIdReply; overload;
    function Add(const ACode, AText: AnsiString): TIdReply; overload;
    constructor Create(AOwner: TPersistent; const AReplyClass: TIdReplyClass); reintroduce; virtual;
    function Find(const ACode: AnsiString; AIgnore: TIdReply = nil): TIdReply; virtual;
...
uses
  CWGlobal, CWResourceStringsCore, SysUtils;
...
procedure TIdReply.SetReply(const ACode: Integer; const AText: AnsiString);
begin
...
procedure TIdReply.SetCode(const AValue: AnsiString);
var
...
procedure TIdReply.SetReply(const ACode, AText: AnsiString);
begin
  Code := ACode;
  FText.Text := AText;
end;

function TIdReply.CheckIfCodeIsValid(const ACode: AnsiString): Boolean;
begin
  Result := True;
end;

class function TIdReply.IsEndMarker(const ALine: AnsiString): Boolean;
begin
  Result := False;
end;
...
function TIdReplies.Add(const ACode: Integer; const AText: AnsiString): TIdReply;
begin
  Result := Add(IntToStr(ACode), AText);
end;

function TIdReplies.Add(const ACode, AText: AnsiString): TIdReply;
begin
  Result := Add;
  try
...
function TIdReplies.Find(const ACode: AnsiString; AIgnore: TIdReply = nil): TIdReply;
var
  i: Integer;
begin
  Result := nil;
