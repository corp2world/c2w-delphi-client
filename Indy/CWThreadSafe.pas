...
type
  ...
  public
    procedure Append(const AValue: AnsiString);
    procedure Prepend(const AValue: AnsiString);
    //
  ...
  public
    ...
    procedure AddObject(const AItem: AnsiString; AObject: TObject);
    procedure Clear;
    function Empty: Boolean;
    function Lock: TStringList; reintroduce;
    function ObjectByItem(const AItem: AnsiString): TObject;
    procedure Remove(const AItem: AnsiString);
...
procedure TIdThreadSafeString.Append(const AValue: AnsiString);
begin
  Lock; try
...
function TIdThreadSafeString.GetValue: AnsiString;
begin
  Lock; try
    Result := FValue;
  finally Unlock; end;
...
procedure TIdThreadSafeString.Prepend(const AValue: AnsiString);
begin
  Lock; try
...
procedure TIdThreadSafeString.SetValue(const AValue: AnsiString);
begin
  Lock; try
    FValue := AValue;
  finally Unlock; end;
...
procedure TIdThreadSafeStringList.Add(const AItem: AnsiString);
begin
  with Lock do try
    Add(AItem);
  finally Unlock; end;
...
procedure TIdThreadSafeStringList.AddObject(const AItem: AnsiString; AObject: TObject);
begin
  with Lock do try
...
function TIdThreadSafeStringList.GetValue(const AName: AnsiString): AnsiString;
begin
  with Lock do try
    Result := Values[AName];
  finally Unlock; end;
...
function TIdThreadSafeStringList.ObjectByItem(const AItem: AnsiString): TObject;
var
  i: Integer;
begin
  Result := nil;
...
procedure TIdThreadSafeStringList.Remove(const AItem: AnsiString);
var
  i: Integer;
begin
  with Lock do try
...
procedure TIdThreadSafeStringList.SetValue(const AName, AValue: AnsiString);
begin
  with Lock do try
    Values[AName] := AValue;
  finally Unlock; end;
