...
type
  ...
  protected
    ...
    FNonce: AnsiString;
    FNonceCount: integer;
    FAlgorithm: AnsiString;
    FMethod, FUri: AnsiString; //needed for digest
    FEntityBody: AnsiString; //needed for auth-int, Somebody make this nice :D
  ...
  public
    ...
    procedure SetRequest(const AMethod, AUri: AnsiString); override;
    property Method: AnsiString read FMethod write FMethod;
    property Uri: AnsiString read FUri write FUri;
    property EntityBody: AnsiString read FEntityBody write FEntityBody;
  end;
...
uses
  CWGlobalProtocols, CWFIPS, CWHash, CWResourceStrings, CWResourceStringsProtocols,
  SysUtils;
...
procedure TIdDigestAuthentication.SetRequest(const AMethod, AUri: AnsiString);
begin
  FMethod := AMethod;
  FUri := AUri;
end;

function TIdDigestAuthentication.Authentication: AnsiString;

  function Hash(const S: AnsiString): AnsiString;
  begin
...
var
  LA1, LA2, LCNonce, LResponse, LQop: AnsiString;
begin
...
function Unquote(var S: AnsiString): AnsiString;
var
  I, Len: Integer;
begin
  Len := Length(S);
...
var
  S, LName, LValue, LTempNonce: AnsiString;
  LParams: TStringList;
begin
  Result := wnDoRequest;
