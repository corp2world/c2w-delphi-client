...
type
  ...
  TIdSSLCipher = class;
  TCallbackEvent  = procedure(const AMsg: AnsiString) of object;
  TCallbackExEvent = procedure(ASender : TObject; const AsslSocket: PSSL;
    const AWhere, Aret: TIdC_INT; const AType, AMsg : AnsiString ) of object;
  ...
  published
    ...
    property DHParamsFile: AnsiString read fsDHParamsFile write fsDHParamsFile;
...
//    property VerifyFile: AnsiString read fVerifyFile write fVerifyFile;
  ...
  protected
    ...
    fSSLVersions : TIdSSLVersions;
    fMode: TIdSSLMode;
    fsRootCertFile, fsCertFile, fsKeyFile, fsDHParamsFile: AnsiString;
    fVerifyDepth: Integer;
    fVerifyMode: TIdSSLVerifyModeSet;
//    fVerifyFile: AnsiString;
    fVerifyDirs: AnsiString;
    fCipherList: AnsiString;
    fContext: PSSL_CTX;
...
//    property VerifyFile: AnsiString read fVerifyFile write fVerifyFile;
  ...
  public
    ...
    function GetSessionIDAsString:AnsiString;
    procedure SetCipherList(CipherList: AnsiString);
    //
  ...
  protected
    ...
    procedure DoStatusInfo(const AMsg: AnsiString); virtual;
    procedure DoStatusInfoEx(const AsslSocket: PSSL;
    const AWhere, Aret: TIdC_INT; const AWhereStr, ARetStr : AnsiString );
...
//    function Accept(ASocket: TIdSocketHandle; AThread: TIdThread) : TIdIOHandler;  override;
  ...
  protected
    ...
    function CertInOneLine: AnsiString;
    function GetHash: TIdSSLULong;
    function GetHashAsString: AnsiString;
  public
    constructor Create(aX509Name: PX509_NAME);
    //
    property Hash: TIdSSLULong read GetHash;
    property HashAsString: AnsiString read GetHashAsString;
    property OneLine: AnsiString read CertInOneLine;
  end;
  ...
  protected
    ...
    function GetMD5AsString:AnsiString;
    function GetSHA1: TIdSSLEVP_MD;
    function GetSHA1AsString:AnsiString;
    function GetSHA224 : TIdSSLEVP_MD;
    function GetSHA224AsString : AnsiString;
    function GetSHA256 : TIdSSLEVP_MD;
    function GetSHA256AsString : AnsiString;
    function GetSHA384 : TIdSSLEVP_MD;
    function GetSHA384AsString : AnsiString;
    function GetSHA512 : TIdSSLEVP_MD;
    function GetSHA512AsString : AnsiString;
  public
     property MD5 : TIdSSLEVP_MD read GetMD5;
     property MD5AsString : AnsiString read  GetMD5AsString;
{IMPORTANT!!!
...
http://csrc.nist.gov/CryptoToolkit/tkhash.html
    ...
    property SHA1AsString : AnsiString read  GetSHA1AsString;
    property SHA224 : TIdSSLEVP_MD read GetSHA224;
    property SHA224AsString : AnsiString read GetSHA224AsString;
    property SHA256 : TIdSSLEVP_MD read GetSHA256;
    property SHA256AsString : AnsiString read GetSHA256AsString;
    property SHA384 : TIdSSLEVP_MD read GetSHA384;
    property SHA384AsString : AnsiString read GetSHA384AsString;
    property SHA512 : TIdSSLEVP_MD read GetSHA512;
    property SHA512AsString : AnsiString read GetSHA512AsString;
  end;
  ...
  protected
    ...
    function GetSigTypeAsString : AnsiString;
  public
    property Signature : AnsiString read GetSignature;
    property SigType : TIdC_INT read  GetSigType ;
    property SigTypeAsString : AnsiString read GetSigTypeAsString;
  end;
  ...
  protected
    ...
    function RFingerprintAsString:AnsiString;
  ...
  public
    ...
    property FingerprintAsString: AnsiString read RFingerprintAsString;
    ...
    property SerialNumber : AnsiString read GetSerialNumber;
...
procedure GetStateVars(const sslSocket: PSSL; AWhere, Aret: TIdC_INT; var VTypeStr, VMsg : AnsiString);
...
begin
  case AWhere of
    ...
    begin
      ...
      VMsg := AnsiString(SSL_alert_type_string_long(Aret));
    end;
    SSL_CB_READ_ALERT :
    begin
      VTypeStr := IndyFormat(RSOSSLReadAlert,[SSL_alert_type_string_long(Aret)]);
      VMsg := AnsiString( SSL_alert_desc_string_long(Aret));
    end;
    SSL_CB_WRITE_ALERT :
    begin
    ...
    begin
      ...
      VMsg := AnsiString( SSL_state_string_long(sslSocket));
    end;
    SSL_CB_ACCEPT_EXIT :
    begin
...
function IndySSL_load_client_CA_file(const AFileName: AnsiString) : PSTACK_OF_X509_NAME; forward;
function IndySSL_CTX_use_PrivateKey_file(ctx: PSSL_CTX; const AFileName: AnsiString;
  AType: Integer): TIdC_INT; forward;
function IndySSL_CTX_use_certificate_file(ctx: PSSL_CTX; const AFileName: AnsiString;
  AType: Integer): TIdC_INT; forward;
//function IndyX509_STORE_load_locations(ctx: PX509_STORE;
//  const AFileName, APathName: AnsiString): TIdC_INT; forward;
function IndySSL_CTX_load_verify_locations(ctx: PSSL_CTX;
  const ACAFile, ACAPath: AnsiString): TIdC_INT; forward;
function IndySSL_CTX_use_DHparams_file(ctx: PSSL_CTX;
  const AFileName: AnsiString; AType: Integer): TIdC_INT; forward;
...
function Indy_unicode_X509_load_cert_crl_file(ctx: PX509_LOOKUP; const AFileName: AnsiString;
  const _type: TIdC_INT): TIdC_INT; forward;
function Indy_unicode_X509_load_cert_file(ctx: PX509_LOOKUP; const AFileName: AnsiString;
...
function IndySSL_load_client_CA_file(const AFileName: AnsiString): PSTACK_OF_X509_NAME;
var
  LM: TMemoryStream;
  LB: PBIO;
...
function IndySSL_CTX_use_certificate_file(ctx: PSSL_CTX;
  const AFileName: AnsiString; AType: Integer): TIdC_INT;
var
  LM: TMemoryStream;
  B: PBIO;
  LX: PX509;
...
//function IndyX509_STORE_load_locations(ctx: PX509_STORE;
//  const AFileName, APathName: AnsiString): TIdC_INT;
//var
//  lookup: PX509_LOOKUP;
//begin
//  Result := 0;
...
function IndySSL_CTX_load_verify_locations(ctx: PSSL_CTX;
  const ACAFile, ACAPath: AnsiString): TIdC_INT;
{$IFDEF USE_INLINE} inline; {$ENDIF}
begin
...
//begin
  ...
  // for unused values, but casting a AnsiString directly to a PAnsiChar
  // always produces a non-nil pointer, which causes X509_STORE_load_locations()
  // to fail. Need to cast the AnsiString to an intermediate Pointer so the
...
function BytesToHexString(APtr: Pointer; ALen: Integer): AnsiString;
{$IFDEF USE_INLINE} inline; {$ENDIF}
var
  i: Integer;
  LPtr: PByte;
...
function MDAsString(const AMD: TIdSSLEVP_MD): AnsiString;
{$IFDEF USE_INLINE} inline; {$ENDIF}
var
  i: Integer;
begin
...
begin
  ...
  try
    ...
    Result := CWSSLOpenSSLHeaders.Load;
...
begin
  Result := '';
  if CWSSLOpenSSL.LoadOpenSSLLibrary then
  begin
    Result := AnsiString(_SSLeay_version(SSLEAY_VERSION));
  end;
end;
...
procedure TIdServerIOHandlerSSLOpenSSL.DoStatusInfo(const AMsg: AnsiString);
begin
...
procedure TIdSSLIOHandlerSocketOpenSSL.DoStatusInfo(const AMsg: AnsiString);
begin
  if Assigned(fOnStatusInfo) then begin
    fOnStatusInfo(AMsg);
  end;
...
{
function TIdSSLContext.LoadVerifyLocations(FileName: AnsiString; Dirs: AnsiString): Boolean;
begin
  Result := False;
...
function  TIdSSLSocket.GetSessionIDAsString:AnsiString;
var
...
procedure TIdSSLSocket.SetCipherList(CipherList: AnsiString);
//var
//  tmpPStr: PAnsiChar;
begin
{
...
function TIdX509Name.CertInOneLine: AnsiString;
var
...
begin
  if FX509Name = nil then begin
    ...
    Result := AnsiString(StrPas(X509_NAME_oneline(FX509Name, PAnsiChar(@LOneLine), SizeOf(LOneLine))));
  end;
end;
...
function TIdX509Name.GetHashAsString: AnsiString;
begin
...
function TIdX509Fingerprints.GetMD5AsString: AnsiString;
begin
...
function TIdX509Fingerprints.GetSHA1AsString: AnsiString;
begin
...
function TIdX509Fingerprints.GetSHA224AsString : AnsiString;
begin
  if Assigned(EVP_sha224) then begin
...
function TIdX509Fingerprints.GetSHA256AsString : AnsiString;
begin
  if Assigned(EVP_sha256) then begin
...
function TIdX509Fingerprints.GetSHA384AsString : AnsiString;
begin
  if Assigned(EVP_SHA384) then begin
...
function TIdX509Fingerprints.GetSHA512AsString : AnsiString;
begin
  if Assigned(EVP_sha512) then begin
...
function TIdX509SigInfo.GetSignature: AnsiString;
begin
...
function TIdX509SigInfo.GetSigTypeAsString: AnsiString;
begin
  Result := AnsiString(OBJ_nid2ln(SigType));
end;
...
function TIdX509.GetSerialNumber: AnsiString;
var
  LSN : PASN1_INTEGER;
begin
...
function TIdX509.RFingerprintAsString: AnsiString;
begin
...
begin
  Result := AnsiString(StrPas(SSL_CIPHER_description(SSL_get_current_cipher(FSSLSocket.fSSL), PAnsiChar(@Buf[0]), SizeOf(Buf)-1)));
end;

function TIdSSLCipher.GetName:AnsiString;
begin
  Result := AnsiString(StrPas(SSL_CIPHER_get_name(SSL_get_current_cipher(FSSLSocket.fSSL))));
end;
...
function TIdSSLCipher.GetVersion:AnsiString;
begin
  Result := AnsiString(StrPas(SSL_CIPHER_get_version(SSL_get_current_cipher(FSSLSocket.fSSL))));
end;
