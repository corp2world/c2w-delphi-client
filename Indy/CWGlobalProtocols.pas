...
type
  ...
  {$ELSE}
  TIdFileName = AnsiString;
  PIdFileNameChar = PChar;
  {$ENDIF}

  TIdReadLnFunction = function: AnsiString of object;
  TStringEvent = procedure(ASender: TComponent; const AString: AnsiString);
  ...
  public
    ...
    procedure AddMimeType(const Ext, MIMEType: AnsiString; const ARaiseOnError: Boolean = True);
    function GetFileMIMEType(const AFileName: AnsiString): AnsiString;
    function GetDefaultFileExt(const MIMEType: AnsiString): AnsiString;
    procedure LoadFromStrings(const AStrings: TStrings; const MimeSeparator: AnsiChar = '=');    {Do not Localize}
    procedure SaveToStrings(const AStrings: TStrings; const MimeSeparator: AnsiChar = '=');    {Do not Localize}
...
//  procedure BuildMIMETypeMap(dest: TIdStringList);
  ...
  function ABNFToText(const AText : AnsiString) : AnsiString;
  function BinStrToInt(const ABinary: AnsiString): Integer;
  function BreakApart(BaseString, BreakString: AnsiString; StringList: TStrings): TStrings;
  function LongWordToFourChar(AValue : LongWord): AnsiString;
  function CharRange(const AMin, AMax : AnsiChar): AnsiString;
  procedure CommaSeparatedToStringList(AList: TStrings; const Value:AnsiString);
  function CompareDateTime(const ADateTime1, ADateTime2 : TDateTime) : Integer;

  function ContentTypeToEncoding(const AContentType: AnsiString; AQuoteType: TIdHeaderQuotingType): TIdTextEncoding;
  function CharsetToEncoding(const ACharset: AnsiString): TIdTextEncoding;

  function ReadStringAsContentType(AStream: TStream; const AContentType: AnsiString;
    AQuoteType: TIdHeaderQuotingType
    {$IFDEF STRING_IS_ANSI}; ADestEncoding: TIdTextEncoding = nil{$ENDIF}): AnsiString;

  procedure ReadStringsAsContentType(AStream: TStream; AStrings: TStrings;
    const AContentType: AnsiString; AQuoteType: TIdHeaderQuotingType
    {$IFDEF STRING_IS_ANSI}; ADestEncoding: TIdTextEncoding = nil{$ENDIF});

  procedure WriteStringAsContentType(AStream: TStream; const AStr, AContentType: AnsiString;
    AQuoteType: TIdHeaderQuotingType
  ...
  procedure WriteStringAsCharset(AStream: TStream; const AStr, ACharset: AnsiString
    {$IFDEF STRING_IS_ANSI}; ASrcEncoding: TIdTextEncoding = nil{$ENDIF});

  procedure WriteStringsAsCharset(AStream: TStream; const AStrings: TStrings;
    const ACharset: AnsiString
    {$IFDEF STRING_IS_ANSI}; ASrcEncoding: TIdTextEncoding = nil{$ENDIF});

  function ReadStringAsCharset(AStream: TStream; const ACharset: AnsiString
    {$IFDEF STRING_IS_ANSI}; ADestEncoding: TIdTextEncoding = nil{$ENDIF}): AnsiString;

  procedure ReadStringsAsCharset(AStream: TStream; AStrings: TStrings; const ACharset: AnsiString
    {$IFDEF STRING_IS_ANSI}; ADestEncoding: TIdTextEncoding = nil{$ENDIF});
  ...
  function DomainName(const AHost: AnsiString): AnsiString;
  function EnsureMsgIDBrackets(const AMsgID: AnsiString): AnsiString;
  function ExtractHeaderItem(const AHeaderLine: AnsiString): AnsiString;
  function ExtractHeaderSubItem(const AHeaderLine, ASubItem: AnsiString; AQuoteType: TIdHeaderQuotingType): AnsiString;
  function ReplaceHeaderSubItem(const AHeaderLine, ASubItem, AValue: AnsiString; AQuoteType: TIdHeaderQuotingType): AnsiString; overload;
  function ReplaceHeaderSubItem(const AHeaderLine, ASubItem, AValue: AnsiString; var VOld: AnsiString; AQuoteType: TIdHeaderQuotingType): AnsiString; overload;
  function IsHeaderMediaType(const AHeaderLine, AMediaType: AnsiString): Boolean;
  function IsHeaderMediaTypes(const AHeaderLine: AnsiString; const AMediaTypes: array of AnsiString): Boolean;
  function ExtractHeaderMediaType(const AHeaderLine: AnsiString): AnsiString;
  function ExtractHeaderMediaSubType(const AHeaderLine: AnsiString): AnsiString;
  function IsHeaderValue(const AHeaderLine: AnsiString; const AValue: AnsiString): Boolean;
  function FileSizeByName(const AFilename: TIdFileName): Int64;
  {$IFDEF WINDOWS}
  function IsVolume(const APathName : TIdFileName) : Boolean;
  {$ENDIF}
  //MLIST FTP DateTime conversion functions
  function FTPMLSToGMTDateTime(const ATimeStamp : AnsiString):TDateTime;
  function FTPMLSToLocalDateTime(const ATimeStamp : AnsiString):TDateTime;

  function FTPGMTDateTimeToMLS(const ATimeStamp : TDateTime; const AIncludeMSecs : Boolean=True): AnsiString;
  function FTPLocalDateTimeToMLS(const ATimeStamp : TDateTime; const AIncludeMSecs : Boolean=True): AnsiString;

  function GetClockValue : Int64;
  function GetMIMETypeFromFile(const AFile: TIdFileName): AnsiString;
  function GetMIMEDefaultFileExt(const MIMEType: AnsiString): TIdFileName;
  function GetGMTDateByName(const AFileName : TIdFileName) : TDateTime;
  function GmtOffsetStrToDateTime(const S: AnsiString): TDateTime;
  function GMTToLocalDateTime(S: AnsiString): TDateTime;
  function CookieStrToLocalDateTime(S: AnsiString): TDateTime;
  function IdGetDefaultCharSet : TIdCharSet;
  function IntToBin(Value: LongWord): AnsiString;
  function IndyComputerName : AnsiString; // DotNet: see comments regarding GDotNetComputerName below
  function IndyCurrentYear : Integer;

  function IndyStrToBool(const AString: AnsiString): Boolean;
  function IsDomain(const S: AnsiString): Boolean;
  function IsFQDN(const S: AnsiString): Boolean;
  function IsBinary(const AChar : AnsiChar) : Boolean;
  function IsHex(const AChar : AnsiChar) : Boolean;
  function IsHostname(const S: AnsiString): Boolean;
  {$IFDEF STRING_IS_ANSI}
  function IsLeadChar(ACh : AnsiChar): Boolean;
  {$ENDIF}
  function IsTopDomain(const AStr: AnsiString): Boolean;
  ...
  function PadString(const AString : AnsiString; const ALen : Integer; const AChar: AnsiChar): AnsiString;
  function UnquotedStr(const AStr : AnsiString): AnsiString;

  function ProcessPath(const ABasePath: AnsiString; const APath: AnsiString; const APathDelim: AnsiString = '/'): AnsiString;    {Do not Localize}
  function RightStr(const AStr: AnsiString; const Len: Integer): AnsiString;
  ...
  function RPos(const ASub, AIn: AnsiString; AStart: Integer = -1): Integer;
  function IndySetLocalTime(Value: TDateTime): Boolean;

  function StartsWith(const ANSIStr, APattern : AnsiString) : Boolean;

  function StrInternetToDateTime(Value: AnsiString): TDateTime;
  function StrToDay(const ADay: AnsiString): Byte;
  function StrToMonth(const AMonth: AnsiString): Byte;
  ...
  function TwoCharToWord(AChar1, AChar2: AnsiChar): Word;
  function UpCaseFirst(const AStr: AnsiString): AnsiString;
  function UpCaseFirstWord(const AStr: AnsiString): AnsiString;
  function GetUniqueFileName(const APath, APrefix, AExt : AnsiString) : AnsiString;
  {$IFDEF WIN32_OR_WIN64}
  function Win32Type : TIdWin32Type;
  {$ENDIF}
  procedure WordToTwoBytes(AWord : Word; ByteArray: TIdBytes; Index: integer);
  function WordToStr(const Value: Word): AnsiString;
  //moved here so I can IFDEF a DotNET ver. that uses StringBuilder
  function IndyWrapText(const ALine, ABreakStr, ABreakChars : AnsiString; MaxCol: Integer): AnsiString;
 
  //The following is for working on email headers and message part headers...
  function RemoveHeaderEntry(const AHeader, AEntry: AnsiString; AQuoteType: TIdHeaderQuotingType): AnsiString; overload;
  function RemoveHeaderEntry(const AHeader, AEntry: AnsiString; var VOld: AnsiString; AQuoteType: TIdHeaderQuotingType): AnsiString; overload;
  function RemoveHeaderEntries(const AHeader: AnsiString; AEntries: array of AnsiString; AQuoteType: TIdHeaderQuotingType): AnsiString;
  ...
  function FindFirstOf(const AFind, AText: AnsiString; const ALength: Integer = -1; const AStartPos: Integer = 1): Integer;
  function FindFirstNotOf(const AFind, AText: AnsiString; const ALength: Integer = -1; const AStartPos: Integer = 1): Integer;
  function TrimAllOf(const ATrim, AText: AnsiString): AnsiString;
  procedure ParseMetaHTTPEquiv(AStream: TStream; AStr : TStrings);

type
  TIdEncodingNeededEvent = function(const ACharset: AnsiString): TIdTextEncoding;
...
var
  ...
  IndyFalseBoolStrs : array of AnsiString;
  IndyTrueBoolStrs : array of AnsiString;
...
uses
  ...
  {$ENDIF}
  CWAssignedNumbers,
  CWResourceStringsCore,
  CWResourceStringsProtocols,
  CWStack;
...
{$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  VDest := CWGlobal.BytesToWord(ASource, ASourceIndex);
...
{$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  VDest := CWGlobal.BytesToLongWord(ASource, ASourceIndex);
  VDest := GStack.NetworkToHost(VDest);
end;
...
function RawStrInternetToDateTime(var Value: AnsiString; var VDateTime: TDateTime): Boolean;
var
  i: Integer;
  Dt, Mo, Yr, Ho, Min, Sec, MSec: Word;
  sYear, sTime, sDelim: AnsiString;
...
begin
  ...
  try
    ...
    if i > 0 then begin
      // Copy time AnsiString up until next space (before GMT offset)
...
function StrToMonth(const AMonth: AnsiString): Byte;
const
  // RLebeau 1/7/09: using AnsiChar() for #128-#255 because in D2009, the compiler
  ...
  Months: array[0..7] of array[1..12] of AnsiString = (

    // English
    ('JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC'),

    // English - alt. 4 letter abbreviations (Netware Print Services may return a 4 ansichar month such as Sept)
    ('',    '',    '',    '',    '',   'JUNE','JULY', '',   'SEPT', '',    '',    ''),
      ...
      decimal :
        If IsNumeric(AText[i]) then begin
          ...
          if IndyStrToInt(LNum, 0) > $FF then begin
            ...
            Result := Result + AnsiChar(IndyStrToInt(LNum, 0));
            LR := Data;
            Result := Result + AText[i];
          end;
        end else begin
    ...
    binary :
        If IsBinary(AText[i]) and (Length(LNum)<8) then begin
          ...
          if (BinStrToInt(LNum)>$FF) then begin
            IdDelete(LNum,Length(LNum),1);
            Result := Result + AnsiChar(BinStrToInt(LNum));
            LR := Data;
            Result := Result + AText[i];
          end;
        end else begin
...
function TimeZoneToGmtOffsetStr(const ATimeZone: AnsiString): AnsiString;
type
...
var
  ...
  function ExtractDigits(var AStr: AnsiString; MinDigits, MaxDigits: Integer): AnsiString;
  var
    LLength: Integer;
  begin
    Result := '';
  ...
  function ParseTime(const AStr: AnsiString): Boolean;
  var
    S, LTemp: AnsiString;
  begin
    {
  ...
  function ParseDayOfMonth(const AStr: AnsiString): Boolean;
  var
    S, LTemp: AnsiString;
  begin
    {
  ...
  function ParseMonth(const AStr: AnsiString): Boolean;
  begin
    {
  ...
  function ParseYear(const AStr: AnsiString): Boolean;
  var
    S, LTemp: AnsiString;
  begin
  ...
  procedure ProcessToken(const AStr: AnsiString);
  begin
...
{ Takes a cardinal (DWORD)  value and returns the AnsiString representation of it's binary value}    {Do not Localize}
function IntToBin(Value: LongWord): AnsiString;
var
  i: Integer;
begin
...
{$IFDEF UNIX}
procedure LoadMIME(const AFileName : AnsiString; AMIMEList : TStrings);
var
  KeyList: TStringList;
  i, p: Integer;
  s, LMimeType, LExtension: AnsiString;
begin
...
procedure TIdMimeTable.AddMimeType(const Ext, MIMEType: AnsiString; const ARaiseOnError: Boolean = True);
var
  LExt,
  LMIMEType: AnsiString;
begin
...
function TIdMimeTable.GetDefaultFileExt(const MIMEType: AnsiString): AnsiString;
var
  Index : Integer;
  LMimeType: AnsiString;
begin
...
function TIdMimeTable.GetFileMIMEType(const AFileName: AnsiString): AnsiString;
var
  Index : Integer;
  LExt: AnsiString;
begin
...
procedure TIdMimeTable.LoadFromStrings(const AStrings: TStrings; const MimeSeparator: AnsiChar = '=');    {Do not Localize}
var
  I, P: Integer;
  S, Ext: AnsiString;
begin
...
function ProcessPath(const ABasePath: AnsiString;
  const APath: AnsiString;
  const APathDelim: AnsiString = '/'): AnsiString;    {Do not Localize}
...
const
 ...
 HTML_MainDocParts : array [0..2] of AnsiString = ('TITLE','HEAD', 'BODY'); {do not localize}
 HTML_HeadDocAttrs : array [0..3] of AnsiString = ('META','TITLE','SCRIPT','LINK'); {do not localize}
 HTML_MetaAttrs : array [0..1] of AnsiString = ('HTTP-EQUIV', 'charset'); {do not localize}

//function ParseUntilEndOfTag(const AStr : AnsiString; var VPos : Integer;
//  const ALen : Integer): AnsiString; {$IFDEF USE_INLINE}inline;{$ENDIF}
//var
//  LStart: Integer;
//begin
//  LStart := VPos;
...
procedure DiscardUntilEndOfTag(const AStr : AnsiString; var VPos : Integer;
...
function ExtractDocWhiteSpace(const AStr : AnsiString; var VPos : Integer;
  const ALen : Integer) : AnsiString;  {$IFDEF USE_INLINE}inline;{$ENDIF}
var
  LStart: Integer;
begin
...
procedure DiscardDocWhiteSpace(const AStr : AnsiString; var VPos : Integer; const ALen : Integer);  {$IFDEF USE_INLINE}inline; {$ENDIF}
begin
  while VPos <= ALen do begin
    if not CharIsInSet(AStr, VPos, HTML_DOCWHITESPACE) then begin
      Break;
...
function ParseWord(const AStr : AnsiString; var VPos : Integer;
  const ALen : Integer) : AnsiString;  {$IFDEF USE_INLINE}inline;{$ENDIF}
var
  LStart: Integer;
begin
...
//procedure DiscardWord(const AStr : AnsiString; var VPos : Integer;
//  const ALen : Integer);  {$IFDEF USE_INLINE}inline;{$ENDIF}
//begin
//  while VPos <= ALen do begin
//    if not CharIsInSet(AStr, VPos, HTML_ALLOWABLE_ALPHANUMBERIC) then begin
...
function ParseUntil(const AStr : AnsiString; const AChar : AnsiChar;
  var VPos : Integer; const ALen : Integer) : AnsiString;  {$IFDEF USE_INLINE}inline;{$ENDIF}
var
  LStart: Integer;
begin
  LStart := VPos;
...
procedure DiscardUntil(const AStr : AnsiString; const AChar : AnsiChar;
...
function ParseUntilCharOrEndOfTag(const AStr : AnsiString; const AChar: AnsiChar;
  var VPos : Integer; const ALen : Integer): AnsiString; {$IFDEF USE_INLINE}inline;{$ENDIF}
var
  LStart: Integer;
begin
...
procedure DiscardUntilCharOrEndOfTag(const AStr : AnsiString; const AChar: AnsiChar;
  var VPos : Integer; const ALen : Integer); {$IFDEF USE_INLINE}inline;{$ENDIF}
begin
  while VPos <= ALen do begin
    if (AStr[VPos] = AChar) or (AStr[VPos] = '>') then begin {do not localize}
...
function ParseHTTPMetaEquiveData(const AStr : AnsiString; var VPos : Integer;
  const ALen : Integer) : AnsiString;  {$IFDEF USE_INLINE}inline;{$ENDIF}
var
  LQuoteChar : AnsiChar;
  LWord : AnsiString;
...
function ParseMetaCharsetData(const AStr : AnsiString; var VPos : Integer;
  const ALen : Integer) : AnsiString;  {$IFDEF USE_INLINE}inline;{$ENDIF}
var
  LQuoteChar : AnsiChar;
  LWord : AnsiString;
...
procedure DiscardToEndOfComment(const AStr : AnsiString; var VPos : Integer; const ALen : Integer);  {$IFDEF USE_INLINE}inline; {$ENDIF}
var
  i : Integer;
begin
...
//function ParseForCloseTag(const AStr, ATagWord : AnsiString; var VPos : Integer; const ALen : Integer) : AnsiString; {$IFDEF USE_INLINE}inline; {$ENDIF}
//var
//  LWord, LTmp : AnsiString;
//begin
//  Result := '';
//  while VPos <= ALen do begin
...
procedure DiscardUntilCloseTag(const AStr, ATagWord : AnsiString; var VPos : Integer;
...
const
  QuoteSpecials: array[TIdHeaderQuotingType] of AnsiString = (
...
procedure SplitHeaderSubItems(AHeaderLine: AnsiString; AItems: TStrings;
  AQuoteType: TIdHeaderQuotingType);
var
  LName, LValue, LSep: AnsiString;
  I: Integer;
...
function ExtractHeaderSubItem(const AHeaderLine, ASubItem: AnsiString;
  AQuoteType: TIdHeaderQuotingType): AnsiString;
var
  LItems: TStringList;
...
function ReplaceHeaderSubItem(const AHeaderLine, ASubItem, AValue: AnsiString;
  AQuoteType: TIdHeaderQuotingType): AnsiString;
var
  LOld: AnsiString;
begin
...
function ReplaceHeaderSubItem(const AHeaderLine, ASubItem, AValue: AnsiString;
  var VOld: AnsiString; AQuoteType: TIdHeaderQuotingType): AnsiString;
var
  LItems: TStringList;
  I: Integer;
  ...
  var
    I: Integer;
    LAddQuotes: Boolean;
    LNeedQuotes, LNeedEscape: AnsiString;
  begin
    Result := '';
    if Length(S) = 0 then begin
      Exit;
...
function MediaTypeMatches(const AValue, AMediaType: AnsiString): Boolean;
begin
...
function IndyComputerName: AnsiString;
{$IFDEF DOTNET}
  {$IFDEF USE_INLINE} inline; {$ENDIF}
{$ENDIF}
{$IFDEF UNIX}
...
begin
  ...
  // i.e. DotNet native AnsiString is 2 byte Unicode, we do not concern ourselves
  ...
  case SysLocale.PriLangID of
    ...
    LANG_UKRAINIAN: Result := idcs_windows_1251;
      ...
      Result := idcs_UNICODE_1_1;
      // not a particular Unicode encoding - just unicode in general
      // i.e. Delphi/C++Builder 2009+ native AnsiString is 2 byte Unicode,
...
function RemoveHeaderEntry(const AHeader, AEntry: AnsiString;
  AQuoteType: TIdHeaderQuotingType): AnsiString;
{$IFDEF USE_INLINE}inline;{$ENDIF}
begin
...
function RemoveHeaderEntry(const AHeader, AEntry: AnsiString; var VOld: AnsiString;
  AQuoteType: TIdHeaderQuotingType): AnsiString;
{$IFDEF USE_INLINE}inline;{$ENDIF}
begin
...
function RemoveHeaderEntries(const AHeader: AnsiString; AEntries: array of AnsiString;
  AQuoteType: TIdHeaderQuotingType): AnsiString;
var
  I: Integer;
begin
...
function FindFirstOf(const AFind, AText: AnsiString; const ALength: Integer = -1;
...
function FindFirstNotOf(const AFind, AText: AnsiString; const ALength: Integer = -1;
  const AStartPos: Integer = 1): Integer;
var
  I, LLength, LPos: Integer;
begin
...
function ContentTypeToEncoding(const AContentType: AnsiString;
