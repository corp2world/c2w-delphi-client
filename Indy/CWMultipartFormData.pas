...
}
{
  ...
  Rev 1.2    10/17/2003 12:49:52 AM  DSiders
    ...
    Added resource AnsiString for unsupported operation exception.
...
type
  ...
  protected
    ...
    FContentTransfer: AnsiString;
    FFieldName: AnsiString;
    FFieldStream: TStream;
    FFieldValue: AnsiString;
    FCanFreeFieldStream: Boolean;
    FHeaderCharSet: AnsiString;
    FHeaderEncoding: Char;

    function FormatHeader: AnsiString;
    ...
    procedure SetContentTransfer(const Value: AnsiString);
    procedure SetFieldName(const Value: AnsiString);
    procedure SetFieldStream(const Value: TStream);
    procedure SetFieldValue(const Value: AnsiString);
    procedure SetFileName(const Value: AnsiString);
    procedure SetHeaderCharSet(const Value: AnsiString);
    procedure SetHeaderEncoding(const Value: Char);
  public
    constructor Create(Collection: TCollection); override;
    destructor Destroy; override;
    // procedure Assign(Source: TPersistent); override;
    property ContentTransfer: AnsiString read FContentTransfer write SetContentTransfer;
    property ContentType: AnsiString read FContentType write SetContentType;
    ...
    property FieldValue: AnsiString read GetFieldValue write SetFieldValue;
    property FieldSize: Int64 read GetFieldSize;
    property HeaderCharSet: AnsiString read FHeaderCharSet write SetHeaderCharSet;
  ...
  public
    ...
    function AddFormField(const AFieldName, AFieldValue: AnsiString; const ACharset: AnsiString = ''; const AContentType: AnsiString = ''; const AFileName: AnsiString = ''): TIdFormDataField; overload;
    function AddFormField(const AFieldName, AContentType, ACharset: AnsiString; AFieldValue: TStream; const AFileName: AnsiString = ''): TIdFormDataField; overload;
    //function AddObject(const AFieldName, AContentType, ACharset: AnsiString; AFileData: TObject; const AFileName: AnsiString = ''): TIdFormDataField; {$IFDEF HAS_DEPRECATED}deprecated{$IFDEF HAS_DEPRECATED_MSG} 'Use overloaded version of AddFormField()'{$ENDIF};{$ENDIF}
    function AddFile(const AFieldName, AFileName: AnsiString; const AContentType: AnsiString = ''): TIdFormDataField;
...
uses
  SysUtils,
  CWCoderQuotedPrintable,
  CWCoderMIME,
  CWStream,
  CWGlobalProtocols;

const
  cAllowedContentTransfers: array[0..4] of AnsiString = (
    '7bit', '8bit', 'binary', 'quoted-printable', 'base64' {do not localize}
    );

  cAllowedHeaderEncodings: array[0..2] of AnsiString = (
...
{function TIdMultiPartFormDataStream.AddObject(const AFieldName,
  AContentType, ACharset: AnsiString; AFileData: TObject;
  const AFileName: AnsiString = ''): TIdFormDataField;
begin
...
function TIdMultiPartFormDataStream.AddFile(const AFieldName, AFileName: AnsiString;
  const AContentType: AnsiString = ''): TIdFormDataField;
var
...
function TIdMultiPartFormDataStream.AddFormField(const AFieldName, AFieldValue: AnsiString;
  const ACharset: AnsiString = ''; const AContentType: AnsiString = ''; const AFileName: AnsiString = ''): TIdFormDataField;
var
  LItem: TIdFormDataField;
begin
  LItem := FFields.Add;
...
function TIdMultiPartFormDataStream.AddFormField(const AFieldName, AContentType, ACharset: AnsiString;
  AFieldValue: TStream; const AFileName: AnsiString = ''): TIdFormDataField;
var
  LItem: TIdFormDataField;
begin
...
function TIdMultiPartFormDataStream.GenerateUniqueBoundary: AnsiString;
begin
...
function TIdFormDataField.FormatHeader: AnsiString;
var
  LBoundary: AnsiString;
begin
  ...
  if Assigned(FFieldStream) then begin
    ...
    {$ENDIF}
      ...
      if I = 0 then begin
        ...
        // AnsiString to find out the final length...
...
function TIdFormDataField.GetFieldValue: AnsiString;
begin
  if Assigned(FFieldStream) then begin
    raise EIdInvalidObjectType.Create(RSMFDInvalidObjectType);
  end;
...
procedure TIdFormDataField.SetCharset(const Value: AnsiString);
begin
  FCharset := Value;
end;

procedure TIdFormDataField.SetContentTransfer(const Value: AnsiString);
begin
...
procedure TIdFormDataField.SetContentType(const Value: AnsiString);
var
  LContentType, LCharSet: AnsiString;
begin
  if Length(Value) > 0 then begin
    LContentType := Value;
  end
...
procedure TIdFormDataField.SetFieldName(const Value: AnsiString);
begin
  FFieldName := Value;
end;
...
procedure TIdFormDataField.SetFieldValue(const Value: AnsiString);
begin
  if Assigned(FFieldStream) then begin
    if FCanFreeFieldStream then begin
      FFieldStream.Free;
...
procedure TIdFormDataField.SetFileName(const Value: AnsiString);
begin
  FFileName := ExtractFileName(Value);
end;

procedure TIdFormDataField.SetHeaderCharSet(const Value: AnsiString);
begin
