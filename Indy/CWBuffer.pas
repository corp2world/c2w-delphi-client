...
}
{
  ...
  Added empty AnsiString return value to Extract() when AByteCount <= 0
...

{
  ...
  example if we have a AnsiString it must be converted to bytes. This conversion
  ...
  accessed the memory directly from the AnsiString. This avoided the overhead of a
  copy.
...
type
  ...
  public
    ...
    function IndexOf(const AString: AnsiString; AStartPos: Integer = 0;
    ...
    // Write
    procedure Write(const AString: AnsiString; AByteEncoding: TIdTextEncoding = nil;
    ...
    //useful for testing. returns buffer as AnsiString without extraction.
...
uses
  ...
  CWStream,
  CWStack; //needed for byte order functions
...
function TIdBuffer.IndexOf(const AString: AnsiString; AStartPos: Integer = 0;
  AByteEncoding: TIdTextEncoding = nil
  {$IFDEF STRING_IS_ANSI}; ASrcEncoding: TIdTextEncoding = nil{$ENDIF}
  ): Integer;
begin
...
procedure TIdBuffer.Write(const AString: AnsiString; AByteEncoding: TIdTextEncoding = nil;
  const ADestIndex : Integer = -1
  {$IFDEF STRING_IS_ANSI}; ASrcEncoding: TIdTextEncoding = nil{$ENDIF}
  );
begin
...
function TIdBuffer.GetAsString: AnsiString;
begin
