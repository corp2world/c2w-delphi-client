...
type
   ...
   protected
     ...
     function ConvertAddress: AnsiString;
     function GetDomain: AnsiString;
     procedure SetDomain(const ADomain: AnsiString);
     function GetUsername: AnsiString;
     procedure SetUsername(const AUsername: AnsiString);
   public
     procedure Assign(Source: TPersistent); override;
     constructor Create; reintroduce; overload;
     constructor Create(ACollection: TCollection); overload; override;
     constructor Create(const AText: AnsiString); reintroduce; overload;
   published
     ...
     property Domain: AnsiString read GetDomain write SetDomain;
     property User: AnsiString read GetUsername write SetUsername;
   end;
   ...
   protected
     ...
     function GetEMailAddresses: AnsiString;
     procedure SetEMailAddresses(AList: AnsiString);
   public
     ...
     procedure AddressesByDomain(AList: TIdEMailAddressList; const ADomain: AnsiString);
     ...
     { Comma-separated list of formated addresses including the names
       from the collection }
     property EMailAddresses: AnsiString read GetEMailAddresses write SetEMailAddresses;
   end;
...
uses
  ...
  CWGlobalProtocols,
  CWExceptionCore,
  CWResourceStringsProtocols, SysUtils;

const
  // ATEXT without the double quote and space characters
  IETF_ATEXT: AnsiString = 'abcdefghijklmnopqrstuvwxyz' + {do not localize}
    'ABCDEFGHIJKLMNOPQRSTUVWXYZ' +                    {do not localize}
    '1234567890!#$%&''*+-/=?_`{}|~';                  {do not localize}

  // ATEXT without the double quote character
  IETF_ATEXT_SPACE: AnsiString = 'abcdefghijklmnopqrstuvwxyz' + {do not localize}
    'ABCDEFGHIJKLMNOPQRSTUVWXYZ' +                          {do not localize}
    '1234567890!#$%&''*+-/=?_`{}|~ ';                       {do not localize}

  IETF_QUOTABLE: AnsiString = '\"'; {do not localize}
...
constructor TIdEMailAddressItem.Create(const AText: AnsiString);
begin
  inherited Create(nil);
  Text := AText;
end;
...
function TIdEMailAddressItem.ConvertAddress: AnsiString;
var
  i: Integer;
  domainPart, tempAddress, localPart: AnsiString;
begin
  if FAddress = '' then
  begin
    if FName <> '' then
...
function TIdEMailAddressItem.GetDomain: AnsiString;
var
  i: Integer;
begin
  Result := '';
...
procedure TIdEMailAddressItem.SetDomain(const ADomain: AnsiString);
var
 S : AnsiString;
 lPos: Integer;
begin
...
function TIdEMailAddressItem.GetUsername: AnsiString;
var
  i: Integer;
begin
  Result := '';
...
procedure TIdEMailAddressItem.SetUsername(const AUsername: AnsiString);
var
 S : AnsiString;
 lPos: Integer;
begin
...
function TIdEMailAddressItem.GetText: AnsiString;
var
  i: Integer;
  tempName, resName: AnsiString;
begin
...
procedure TIdEMailAddressItem.SetText(AText: AnsiString);
var
  nFirst,
...
function TIdEMailAddressList.GetEMailAddresses: AnsiString;
var
  idx: Integer;
begin
  Result := '';   {Do not Localize}
...
procedure TIdEMailAddressList.SetEMailAddresses(AList: AnsiString);
var
