...
function CalcUTF16CharLength(const AStr: {$IFDEF STRING_IS_UNICODE}AnsiString{$ELSE}TIdWideChars{$ENDIF}; const AIndex: Integer): Integer;
function WideCharIsInSet(const ASet: TIdUnicodeString; const AChar: WideChar): Boolean;
function GetUTF16Codepoint(const AStr: {$IFDEF STRING_IS_UNICODE}AnsiString{$ELSE}TIdWideChars{$ENDIF}; const AIndex: Integer): Integer;
...
uses
  ...
  CWResourceStringsUriUtils;
  {$ENDIF}
{$ENDIF}

function CalcUTF16CharLength(const AStr: {$IFDEF STRING_IS_UNICODE}AnsiString{$ELSE}TIdWideChars{$ENDIF};
...
function GetUTF16Codepoint(const AStr: {$IFDEF STRING_IS_UNICODE}AnsiString{$ELSE}TIdWideChars{$ENDIF};
  const AIndex: Integer): Integer;
{$IFDEF DOTNET}
  {$IFDEF USE_INLINE}inline;{$ENDIF}
{$ELSE}
