{
 ...
 Minor optimizations.  Illiminated some unnecessary AnsiString operations.
...
uses
  Classes,
  CWGlobal, CWException, CWStackBSDBase, CWStackConsts, CWWinsock2, CWStack,
  SysUtils, 
  Windows;
...
type
  ...
  protected
     procedure WSQuerryIPv6Route(ASocket: TIdStackSocketHandle;
       const AIP: AnsiString; const APort : Word; var VSource; var VDest);
    procedure WriteChecksumIPv6(s : TIdStackSocketHandle; var VBuffer : TIdBytes;
      const AOffset : Integer; const AIP : AnsiString; const APort : TIdPort);
    function HostByName(const AHostName: AnsiString;
      const AIPVersion: TIdIPVersion = ID_DEFAULT_IP_VERSION): AnsiString; override;
    function ReadHostName: AnsiString; override;
  ...
  public
    function Accept(ASocket: TIdStackSocketHandle; var VIP: AnsiString; var VPort: TIdPort;
    ...
    //
    function HostByAddress(const AAddress: AnsiString;
      const AIPVersion: TIdIPVersion = ID_DEFAULT_IP_VERSION): AnsiString; override;

    function WSGetServByName(const AServiceName: AnsiString): TIdPort; override;
    ...
    function RecvFrom(const ASocket: TIdStackSocketHandle; var VBuffer;
     const ALength, AFlags: Integer; var VIP: AnsiString; var VPort: TIdPort;
   ...
   function ReceiveMsg(ASocket: TIdStackSocketHandle; var VBuffer: TIdBytes;
    ...
    procedure WSSendTo(ASocket: TIdStackSocketHandle; const ABuffer;
      const ABufferLength, AFlags: Integer; const AIP: AnsiString; const APort: TIdPort; AIPVersion: TIdIPVersion = ID_DEFAULT_IP_VERSION); override;
    ...
    function WSTranslateSocketErrorMsg(const AErr: integer): AnsiString; override;
    ...
    //
    procedure Bind(ASocket: TIdStackSocketHandle; const AIP: AnsiString;
     const APort: TIdPort; const AIPVersion: TIdIPVersion = ID_DEFAULT_IP_VERSION); override;
    procedure Connect(const ASocket: TIdStackSocketHandle; const AIP: AnsiString;
     const APort: TIdPort; const AIPVersion: TIdIPVersion = ID_DEFAULT_IP_VERSION); override;
    constructor Create; override;
    destructor Destroy; override;
    procedure Disconnect(ASocket: TIdStackSocketHandle); override;
    procedure GetPeerName(ASocket: TIdStackSocketHandle; var VIP: AnsiString;
     var VPort: TIdPort; var VIPVersion: TIdIPVersion); override;
    procedure GetSocketName(ASocket: TIdStackSocketHandle; var VIP: AnsiString;
     var VPort: TIdPort; var VIPVersion: TIdIPVersion); override;
...
uses
  CWIDN, CWResourceStrings, CWWship6;
...
begin
  ...
  if not GStarted then begin
    try
      InitializeWinSock;
      CWWship6.InitLibrary;
      CWIDN.InitIDNLibrary;
    except
...
function TIdStackWindows.Accept(ASocket: TIdStackSocketHandle;
  var VIP: AnsiString; var VPort: TIdPort; var VIPVersion: TIdIPVersion): TIdStackSocketHandle;
var
  LSize: Integer;
  LAddr: TSockAddrIn6;
begin
  LSize := SizeOf(LAddr);
  Result := CWWinsock2.accept(ASocket, PSOCKADDR(@LAddr), @LSize);
...
procedure TIdStackWindows.Bind(ASocket: TIdStackSocketHandle;
  const AIP: AnsiString; const APort: TIdPort;
...
begin
  ...
  CheckForSocketError(CWWinsock2.bind(ASocket, PSOCKADDR(@LAddr), LSize));
end;
...
function TIdStackWindows.HostByAddress(const AAddress: AnsiString;
  const AIPVersion: TIdIPVersion = ID_DEFAULT_IP_VERSION): AnsiString;
var
  Host: PHostEnt;
  {$IFNDEF WINCE}
  LAddr: u_long;
...
function TIdStackWindows.ReadHostName: AnsiString;
var
...
begin
  ...
  //we have to specifically type cast a PAnsiChar to a AnsiString for D2009+.
  //otherwise, we will get a warning about implicit typecast from AnsiString
  //to AnsiString
  Result := AnsiString(PAnsiChar(LStr));
end;

procedure TIdStackWindows.Listen(ASocket: TIdStackSocketHandle; ABackLog: Integer);
begin
  CheckForSocketError(CWWinsock2.listen(ASocket, ABacklog));
end;
...
function TIdStackWindows.RecvFrom(const ASocket: TIdStackSocketHandle;
  var VBuffer; const ALength, AFlags: Integer; var VIP: AnsiString;
...
begin
  LSize := SizeOf(LAddr);
  Result := CWWinsock2.recvfrom(ASocket, VBuffer, ALength, AFlags, PSOCKADDR(@LAddr), @LSize);
  if Result >= 0 then
  begin
    case LAddr.sin6_family of
      Id_PF_INET4: begin
...
begin
  Result := CheckForSocketError(CWWinsock2.send(ASocket, ABuffer, ABufferLength, AFlags));
end;

procedure TIdStackWindows.WSSendTo(ASocket: TIdStackSocketHandle;
  const ABuffer; const ABufferLength, AFlags: Integer; const AIP: AnsiString;
...
begin
  ...
  LSize := CWWinsock2.sendto(ASocket, ABuffer, ABufferLength, AFlags, PSOCKADDR(@LAddr), LSize);
...
begin
  CheckForSocketError(CWWinsock2.setsockopt(ASocket, ALevel, AOptName, PAnsiChar(@AOptVal), SIZE_INTEGER));
end;
...
begin
  if AOverlapped then begin
    ...
    Result := CWWinsock2.socket(AFamily, AStruct, AProtocol);
  end;
end;

function TIdStackWindows.WSGetServByName(const AServiceName: AnsiString): TIdPort;
var
  ps: PServEnt;
  {$IFDEF STRING_IS_UNICODE}
  LTemp: AnsiString;
...
begin
  ...
  try
    ...
    //otherwise, we will get a warning about implicit typecast from AnsiString
    //to AnsiString
    AAddresses.Add(AnsiString(PAnsiChar(ps^.s_name)));
    i := 0;
    p := Pointer(ps^.s_aliases);
    while p[i] <> nil do
    begin
      AAddresses.Add(AnsiString(PAnsiChar(p[i])));
      Inc(i);
    end;
  finally
...
procedure TIdStackWindows.GetSocketName(ASocket: TIdStackSocketHandle;
  var VIP: AnsiString; var VPort: TIdPort; var VIPVersion: TIdIPVersion);
var
  LSize: Integer;
  LAddr: TSockAddrIn6;
begin
...
function TIdStackWindows.WSTranslateSocketErrorMsg(const AErr: Integer): AnsiString;
begin
...
begin
  ...
  LResult := CWWinsock2.select(0, AReadSet, AWriteSet, AExceptSet, LTimePtr);
...
function TIdStackWindows.HostByName(const AHostName: AnsiString;
  const AIPVersion: TIdIPVersion = ID_DEFAULT_IP_VERSION): AnsiString;
var
  LPa: PAnsiChar;
  LSa: TInAddr;
...
begin
  ...
  begin
    case AIPVersion of
      Id_IPv4:
        begin
          LHost := CWWinsock2.gethostbyname(
            {$IFDEF STRING_IS_UNICODE}
...
begin
  ...
  CheckForSocketError(CWWinsock2.connect(ASocket, PSOCKADDR(@LAddr), LSize));
end;
...
begin
  LSize := SizeOf(LAddr);
  CheckForSocketError(CWWinsock2.getpeername(ASocket, PSOCKADDR(@LAddr), LSize));
  case LAddr.sin6_family of
    Id_PF_INET4: begin
      with PSOCKADDR(@LAddr)^ do
      begin
...
begin
  CheckForSocketError(CWWinsock2.setsockopt(ASocket, ALevel, Aoptname, Aoptval, Aoptlen));
end;
...
begin
  ...
  if (CWWinsock2.WSAEnumProtocols(nil, nil, LLen) = SOCKET_ERROR) and (LLen > 0) then
  begin
    GetMem(LPInfo, LLen);
    try
      LCount := CWWinsock2.WSAEnumProtocols(nil, LPInfo, LLen);
      if LCount > 0 then
      begin
        LPCurPtr := LPInfo;
...
begin
  Result := CWWinsock2.ioctlsocket(s, cmd, arg);
end;

procedure TIdStackWindows.WSQuerryIPv6Route(ASocket: TIdStackSocketHandle;
  const AIP: AnsiString; const APort: TIdPort; var VSource; var VDest);
var
...
procedure TIdStackWindows.WriteChecksum(s: TIdStackSocketHandle;
  var VBuffer: TIdBytes; const AOffset: Integer; const AIP: AnsiString;
...
function ServeFile(ASocket: TIdStackSocketHandle; const AFileName: AnsiString): Int64;
var
  LFileHandle: THandle;
  LSize: LARGE_INTEGER;
  {$IFDEF UNICODE_BUT_STRING_IS_ANSI}
