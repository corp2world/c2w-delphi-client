 ...
 *   + right trim unquoted AnsiString
 ...
 *   + AsString return a AnsiString instead of PChar.
 ...
 *       ex: obj.s['obj.prop[1]'] := 'AnsiString value';
 ...
 *  v0.2
 *   + Hashed AnsiString list replaced with a faster AVL tree
...
{$ENDIF}
    ...
    function SaveTo(const FileName: AnsiString; indent: boolean = false; escape: boolean = true): integer; overload;
...
{$ENDIF}
  ...
  public
    ...
    class function ParseStream(stream: TStream; strict: Boolean; partial: boolean = true; const this: ISuperObject = nil; options: TSuperFindOptions = [];
       const put: ISuperObject = nil; dt: TSuperType = stNull): ISuperObject;
    class function ParseFile(const FileName: AnsiString; strict: Boolean; partial: boolean = true; const this: ISuperObject = nil; options: TSuperFindOptions = [];
       const put: ISuperObject = nil; dt: TSuperType = stNull): ISuperObject;
...
{$IFDEF VER210}
  ...
  TSuperRttiContext = class
  private
    class function GetFieldName(r: TRttiField): AnsiString;
  ...
  public
    ...
    constructor FromJson(const str: AnsiString; ctx: TSuperRttiContext = nil); overload;
  end;
{$ENDIF}
...
function TrySOInvoke(var ctx: TSuperRttiContext; const obj: TValue; const method: AnsiString; const params: ISuperObject; var Return: ISuperObject): TSuperInvokeResult; overload;
function SOInvoke(const obj: TValue; const method: AnsiString; const params: ISuperObject; ctx: TSuperRttiContext = nil): ISuperObject; overload;
function SOInvoke(const obj: TValue; const method: AnsiString; const params: AnsiString; ctx: TSuperRttiContext = nil): ISuperObject; overload;
{$ENDIF}
...
function gcvt(value: Double; ndigit: longint; buf: PAnsiChar): PAnsiChar; cdecl;
  external  'msvcrt.dll' name '_gcvt';
    ...
    case TVarRec(Args[j]).VType of
      ...
      vtClass:
        if TVarRec(Args[j]).VPointer = nil then
          ...
          Add(TSuperObject.Create(PtrInt(TVarRec(Args[j]).VPointer)));
//      vtUnicodeString:
//          Add(TSuperObject.Create(SOString(AnsiString(TVarRec(Args[j]).VUnicodeString))));
    else
      assert(false);
    end;
end;
...
//    varUString:  Result := TSuperObject.Create(SOString(AnsiString(VUString)));
  else
...
function SOInvoke(const obj: TValue; const method: AnsiString; const params: ISuperObject; ctx: TSuperRttiContext): ISuperObject; overload;
var
  owned: Boolean;
begin
  if ctx = nil then
...
{$WARNINGS OFF}
function SOInvoke(const obj: TValue; const method: AnsiString; const params: AnsiString; ctx: TSuperRttiContext): ISuperObject; overload;
begin
...
function TrySOInvoke(var ctx: TSuperRttiContext; const obj: TValue;
  const method: AnsiString; const params: ISuperObject;
...
{$WARNINGS OFF}
class function TSuperObject.ParseFile(const FileName: AnsiString; strict: Boolean;
  partial: boolean; const this: ISuperObject; options: TSuperFindOptions;
  const put: ISuperObject; dt: TSuperType): ISuperObject;
var
  stream: TFileStream;
...
{$WARNINGS OFF}
function TSuperObject.SaveTo(const FileName: AnsiString; indent, escape: boolean): integer;
var
  stream: TFileStream;
begin
...
constructor TSuperAttribute.Create(const AName: AnsiString);
begin
  FName := AName;
end;
...
{$WARNINGS OFF}
class function TSuperRttiContext.GetFieldName(r: TRttiField): AnsiString;
var
  o: TCustomAttribute;
begin
...
function TSuperRttiContext.FromJson(TypeInfo: PTypeInfo; const obj: ISuperObject;
  ...
  begin
    if ObjectIsType(obj, stString) and (Length(obj.AsString) = 1) then
      begin
        Value := AnsiString(AnsiString(obj.AsString)[1]);
        Result := True;
      end else
        Result := False;
  end;
...
{$WARNINGS OFF}
  procedure ToChar;
  begin
    Result := TSuperObject.Create(AnsiString(Value.AsType<AnsiChar>));
  end;
{$WARNINGS ON}
  procedure ToInteger;
  begin
...
{$WARNINGS OFF}
  procedure ToString;
  begin
    Result := TSuperObject.Create(AnsiString(Value.AsType<AnsiString>));
  end;
  ...
  procedure ToWChar;
  begin
    Result :=  TSuperObject.Create(AnsiString(Value.AsType<WideChar>));
  end;
{$WARNINGS ON}
  procedure ToVariant;
  begin
...
{$WARNINGS OFF}
  ...
  begin
    if TValueData(Value).FAsClass <> nil then
      Result :=  TSuperObject.Create(AnsiString(
...
{$WARNINGS OFF}
constructor TSuperObjectHelper.FromJson(const str: AnsiString; ctx: TSuperRttiContext = nil);
begin
  FromJson(SO(str), ctx);
end;
{$WARNINGS ON}
